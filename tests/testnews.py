...
def _load_news_module():
    ...
    _install_stub_module("aiohttp", ClientError=Exception, ClientSession=object)

    _load_module(CONFIG_MODULE_NAME, ROOT / "core" / "config.py")
    return _load_module(NEWS_MODULE_NAME, ROOT / "core" / "news" / "__init__.py")
...
class NewsPayloadDecodingTests(unittest.TestCase):
    def test_decode_payload_rejects_oversized_response(self):
        ...
        with self.assertRaises(json.JSONDecodeError):
            service._loads_json_payload("x" * (8 * 1024 * 1024 + 1))

    def test_decode_payload_bounds_malformed_candidate_scan(self):
        mod = _load_news_module()
        service = mod.NewsService({"news_conf": {}})
        payload = "prefix " + "{x" * 100 + '{"data":[{"title":"不应扫描到"}]}'

        with self.assertRaises(json.JSONDecodeError):
            service._loads_json_payload(payload)
    ...
    def test_decode_payload_prefers_parseable_news_json(self):
        mod = _load_news_module()
        service = mod.NewsService({"news_conf": {}})
        payload = (
            '{"debug":true,"stage":"before"}\n'
            "HTTP Status: 200\n\n"
            '{"code":200,"message":"success","data":[{"title":"目标新闻标题","hot_value":2}]}'
        )

        data = service._loads_json_payload(payload)

        self.assertEqual(data["message"], "success")
        self.assertEqual(data["data"][0]["title"], "目标新闻标题")
        ...
        parsed = service._parse_response(
            {
                "data": [
                    ...
                    },
                    {
                        "title": "标题二",
                        "hot": "热",
                        "mobile_link": "https://example.com/b",
                    },
                ]
            },
            limit=1,
    ...
    def test_parse_response_accepts_kuaishou_hot_items(self):
        mod = _load_news_module()
        service = mod.NewsService({"news_conf": {"news_items_count": 5}})
                ...
                "data": [
                    {
                        ...
                        "hot": 13189000,
                        "url": "https://www.kuaishou.com/short-video/demo",
                    }
                ]
            },
            limit=1,
        ...
        self.assertEqual(parsed[0]["title"], "快手热搜标题")
        self.assertEqual(parsed[0]["hot"], "13189000")
        self.assertEqual(parsed[0]["url"], "https://www.kuaishou.com/short-video/demo")
        self.assertEqual(parsed[0]["cover"], "https://example.com/cover.jpg")


class NewsSessionLifecycleTests(unittest.IsolatedAsyncioTestCase):
    async def test_http_session_is_reused_and_closed(self):
        mod = _load_news_module()
        created = []
        ...
        first = await service.http()
        second = await service.http()
        await service.close()
