...
def _load_qzone_service_module():
    ...
    _install_stub_module(
        ...
        QzoneMediaUploadService=type("QzoneMediaUploadService", (), {}),
    )
    _install_stub_module(
...
class QzoneAutoCommentTests(unittest.IsolatedAsyncioTestCase):
    async def test_transient_query_failure_logs_debug(self):
        ...
        class Owner:
            ...
            async def _qzone_auto_save_state(
                self, state_key, state, processed, result, *, run_at=None
            ):
        ...
        self.assertEqual(
            calls, [("debug", "[日常分享] QQ 空间自动评论查询失败: network busy")]
        )
    ...
    async def test_reply_comment_returns_submitted_target_debug_info(self):
        service_module, models = _load_qzone_service_module()
        service = service_module.QzoneService(
        ...
        post = models.QzonePost(uin=1, tid="self")
        comment = models.QzoneComment(
            uin=2, nickname="Alice", content="second", tid="r2", parent_tid="c1"
        )
        parent = models.QzoneComment(uin=2, nickname="Alice", content="first", tid="c1")
        service._post_cache[post.key] = post
        service.context = lambda: asyncio.sleep(
            0, result=types.SimpleNamespace(uin=1, gtk="1")
        )
        payloads = iter([{"code": 0}])
        service._request = lambda *args, **kwargs: asyncio.sleep(
            0, result=next(payloads)
        )
        service._verify_thread_reply_submission = lambda *args, **kwargs: asyncio.sleep(
            0,
            result={
                "status": "confirmed",
        ...
        result = await service.reply_comment(
            post.key, comment, "reply", parent_comment=parent
        )
    ...
    def test_clean_comment_drops_broken_platform_mention(self):
        ...
        result = module._clean_auto_comment_text(
            "@{uin:100000002,nick:测试用户甲,who:1。", max_bytes=60
        )

        self.assertEqual(result, "")

    def test_clean_comment_treats_model_skip_words_as_empty_generation(self):
        module, _models = _load_auto_comment_module()

        for value in ("跳过", "不评论", "略过", "skip", "PASS"):
            with self.subTest(value=value):
                self.assertEqual(module._clean_auto_comment_text(value), "")
    ...
    def test_detects_self_reply_to_comment(self):
        module, models = _load_auto_comment_module()
        target_comment = models.QzoneComment(
            uin=2, nickname="Alice", content="好看", tid="c1"
        )
        post = models.QzonePost(
            uin=1,
            tid="a",
            comments=[
                target_comment,
                models.QzoneComment(
                    uin=1, content="谢谢你呀", tid="r1", parent_tid="c1"
                ),
            ],
        )
    ...
    def test_detects_reply_targeting_self_by_reply_to_tid(self):
        ...
        post = models.QzonePost(
            ...
            comments=[
                ...
                models.QzoneComment(
                    uin=1, nickname="Me", content="谢谢", tid="r1", parent_tid="c1"
                ),
                models.QzoneComment(
                    uin=2,
                    nickname="Alice",
        ...
        self.assertTrue(
            module._comment_replies_to_self(post.comments[2], 1, index=index)
        )
    ...
    def test_infers_thread_parent_from_reply_target(self):
        ...
        post = models.QzonePost(
            ...
            comments=[
                ...
                models.QzoneComment(
                    uin=2, nickname="Alice", content="哈哈", tid="r2", reply_to_tid="r1"
                ),
            ],
        )
        index = module.QzoneCommentIndex.build(post, 1)
    ...
    def test_thread_candidate_skips_older_friend_reply_when_newer_friend_reply_exists(
        self,
    ):
        module, models = _load_auto_comment_module()
        ...
        post = models.QzonePost(
            ...
            comments=[
                models.QzoneComment(
                    uin=2, nickname="Alice", content="首评", tid="c1", create_time=100
                ),
                models.QzoneComment(
                    uin=1,
                    nickname="Me",
    ...
    def test_summary_generation_failed_suffix_hides_zero_failures(self):
        ...
        self.assertEqual(
            module._qzone_summary_generation_failed_suffix({"generation_failed": 0}), ""
        )
        self.assertEqual(
        ...
        self.assertEqual(
            module._qzone_summary_generation_failed_suffix({"failed": 2}), ""
        )
    ...
    def test_auto_interaction_skips_official_qzone_posts(self):
        ...
        manager = Manager()
        official = models.QzonePost(
            uin=10000, tid="official", name="官方Qzone", text="活动广告"
        )
        friend = models.QzonePost(uin=2, tid="friend", name="朋友", text="今天去了书店")

        self.assertFalse(
            manager._qzone_auto_like_candidate(official, self_uin=1, processed={})
        )
        self.assertFalse(
        ...
        self.assertTrue(
            manager._qzone_auto_like_candidate(friend, self_uin=1, processed={})
        )
        self.assertTrue(
            manager._qzone_auto_comment_candidate(
                friend,
    ...
    async def test_auto_comment_and_reply_system_prompt_includes_persona(self):
        ...
        manager = Manager()
        manager.plugin.call_llm = manager.llm
        friend_post = models.QzonePost(
            uin=2, tid="friend", name="Alice", text="今天去了书店"
        )
        self_post = models.QzonePost(uin=1, tid="self", name="我", text="今天很开心")
        comment = models.QzoneComment(
            uin=2, nickname="Alice", content="真不错", tid="c1"
        )
    ...
    async def test_auto_interaction_prompts_include_life_context(self):
        ...
        manager.plugin.call_llm = manager.llm
        friend_post = models.QzonePost(
            uin=2, tid="friend", name="Alice", text="bookstore afternoon"
        )
        self_post = models.QzonePost(uin=1, tid="self", name="Me", text="quiet day")
        parent_comment = models.QzoneComment(
            uin=2, nickname="Alice", content="nice", tid="c1"
        )
        ...
        await manager._generate_qzone_auto_reply_thread(
            self_post, parent_comment, second_level
        )
        ...
        for prompt in manager.prompts:
            ...
            self.assertNotIn("不要主动透露具体地点、行程、关系、备忘录", prompt)

    async def test_auto_comment_prompt_uses_plain_comment_instruction_by_default(self):
        module, models = _load_auto_comment_module()
        ...
        manager = Manager()
        manager.plugin.call_llm = manager.llm
        post = models.QzonePost(uin=2, tid="friend", name="Alice", text="测试一下")

        result = await manager.generate_qzone_auto_comment(post)

        self.assertEqual(result, "这句挺像深夜突发奇想")
        self.assertIn(
            "给这条好友 QQ 空间动态写一条自然、简短、有人味的评论", manager.prompts[0]
        )
        self.assertIn("优先回应发布者正文和转发语境", manager.prompts[0])
        self.assertIn("配图识别只作为辅助细节", manager.prompts[0])
        self.assertIn("只输出一句自然评论", manager.system_prompts[0])
        self.assertIn(
            "内容优先级为发布者正文、转发语境、配图识别", manager.system_prompts[0]
        )
        self.assertNotIn("没必要互动", manager.prompts[0])
    ...
    async def test_auto_interaction_prompts_include_configured_style_supplements(self):
        ...
        class Manager(module.TaskQzoneAutoCommentService):
            def __init__(self):
                self.qzone_conf = {
                    "qzone_auto_comment_prompt": "像熟人随口一句，别太正式。",
        ...
        self_post = models.QzonePost(uin=1, tid="self", name="Me", text="quiet day")
        comment = models.QzoneComment(uin=2, nickname="Alice", content="nice", tid="c1")
    ...
    async def test_auto_interaction_system_prompts_do_not_request_skip(self):
        ...
        self_post = models.QzonePost(uin=1, tid="self", name="Me", text="quiet day")
        parent = models.QzoneComment(uin=2, nickname="Alice", content="nice", tid="c1")
        target = models.QzoneComment(
            uin=2, nickname="Alice", content="same here", tid="c2", parent_tid="c1"
        )

        await manager.generate_qzone_auto_comment(friend_post)
        await manager._generate_qzone_auto_reply(self_post, parent)
        await manager._generate_qzone_auto_reply_thread(self_post, parent, target)

        self.assertEqual(len(manager.system_prompts), 3)
        for system_prompt in manager.system_prompts:
            self.assertNotIn("跳过", system_prompt)
            self.assertNotIn("或“跳过”", system_prompt)
    ...
    async def test_auto_comment_includes_image_vision_context_when_enabled(self):
        ...
        class Context:
            ...
            def get_using_provider(self):
                return types.SimpleNamespace(
                    meta=lambda: types.SimpleNamespace(id="default-provider")
                )

            async def llm_generate(self, **kwargs):
                self.calls.append(kwargs)
                return types.SimpleNamespace(
                    completion_text="桌上有一杯咖啡和一本打开的书"
                )
        ...
        class Manager(module.TaskQzoneAutoCommentService):
            def __init__(self):
                ...
                self.plugin = types.SimpleNamespace(
                    basic_conf={"llm_provider_id": "ignored-provider"},
        ...
        result = await manager.generate_qzone_auto_comment(post, state=state)

        self.assertEqual(result, "这下午很有生活感")
        self.assertEqual(
            manager.context.calls[0]["chat_provider_id"], "default-provider"
        )
        self.assertEqual(
            manager.context.calls[0]["image_urls"], ["https://example.com/cafe.jpg"]
        )
        self.assertIn("【配图识别】", manager.prompts[0])
        self.assertIn("桌上有一杯咖啡和一本打开的书", manager.prompts[0])
        self.assertIn("image_vision_cache", state)

    async def test_auto_comment_includes_repost_image_vision_context(self):
        module, models = _load_auto_comment_module()
        ...
        class Context:
            ...
            async def llm_generate(self, **kwargs):
                self.calls.append(kwargs)
                return types.SimpleNamespace(
                    completion_text="转发配图里有一只橘猫趴在桌边"
                )
        ...
        post = models.QzonePost(
            uin=2,
            tid="repost-image-post",
            name="FRIEND_NICK",
            text="转一下",
            rt_con="猫猫营业",
            rt_uinname="测试用户B",
            rt_images=["https://example.com/repost-cat.jpg"],
        )

        result = await manager.generate_qzone_auto_comment(post, state={})

        self.assertEqual(result, "这猫看着很会生活")
        self.assertEqual(
            manager.context.calls[0]["image_urls"],
            ["https://example.com/repost-cat.jpg"],
        )
        self.assertIn("发布者正文：转一下", manager.prompts[0])
        self.assertIn("转发来源：测试用户B", manager.prompts[0])
        self.assertIn("转发正文：猫猫营业", manager.prompts[0])
        self.assertIn("媒体：转发图片 1 张", manager.prompts[0])
        self.assertIn("图1（转发配图）", manager.prompts[0])
        self.assertIn("转发配图里有一只橘猫趴在桌边", manager.prompts[0])

    async def test_auto_reply_includes_comment_image_vision_context(self):
        module, models = _load_auto_comment_module()
        ...
        class Context:
            ...
            async def llm_generate(self, **kwargs):
                self.calls.append(kwargs)
                return types.SimpleNamespace(completion_text="评论配图里有一张笑脸自拍")
        ...
        manager.plugin.call_llm = manager.llm
        state = {}
        post = models.QzonePost(uin=1, tid="self", name="Me", text="测试")
        comment = models.QzoneComment(
            uin=2,
            nickname="Alice",
            content="测试评论带图片",
            tid="c1",
            images=["https://example.com/comment-selfie.jpg"],
        )

        result = await manager._generate_qzone_auto_reply(post, comment, state=state)

        self.assertEqual(result, "这张确实笑得很开心")
        self.assertEqual(
            manager.context.calls[0]["image_urls"],
            ["https://example.com/comment-selfie.jpg"],
        )
        self.assertIn("【评论配图识别】", manager.prompts[0])
        self.assertIn("图1（评论配图）", manager.prompts[0])
        self.assertIn("评论配图里有一张笑脸自拍", manager.prompts[0])
        self.assertIn("image_vision_cache", state)

    async def test_auto_reply_thread_prioritizes_new_reply_image_vision_context(self):
        module, models = _load_auto_comment_module()
        ...
        class Context:
            ...
            async def llm_generate(self, **kwargs):
                self.calls.append(kwargs)
                return types.SimpleNamespace(
                    completion_text=f"{kwargs['image_urls'][0]} 的识别结果"
                )
        ...
        class Manager(module.TaskQzoneAutoCommentService):
            def __init__(self):
                self.qzone_conf = {
                    ...
                    "qzone_auto_comment_image_vision_limit": 2,
                }
                self.prompts = []
                self.context = Context()
                self.plugin = types.SimpleNamespace(
        ...
        manager.plugin.call_llm = manager.llm
        post = models.QzonePost(uin=1, tid="self", name="Me", text="测试")
        parent = models.QzoneComment(
            uin=2,
            nickname="Alice",
            content="一级评论",
            tid="c1",
            images=["https://example.com/parent.jpg"],
        )
        target = models.QzoneComment(
            uin=2,
            nickname="Alice",
            ...
            tid="r1",
            parent_tid="c1",
            images=["https://example.com/new-reply.jpg"],
        )

        result = await manager._generate_qzone_auto_reply_thread(
            post, parent, target, state={}
        )

        self.assertEqual(result, "我先接你新发的这张图")
        self.assertEqual(
            [call["image_urls"][0] for call in manager.context.calls],
            ["https://example.com/new-reply.jpg", "https://example.com/parent.jpg"],
        )
        self.assertIn("图1（新的二级回复配图）", manager.prompts[0])
        self.assertIn("图2（一级评论配图）", manager.prompts[0])
    ...
    async def test_auto_comment_image_vision_logs_debug_path(self):
        ...
        with patch.object(
            ...
            await manager.generate_qzone_auto_comment(post, state={})

        self.assertTrue(any("QQ 空间说说配图识别开始" in item for item in logs))
        self.assertTrue(any("QQ 空间说说配图识别成功" in item for item in logs))
    ...
    async def test_auto_comment_image_vision_reuses_state_cache(self):
        ...
        await manager.generate_qzone_auto_comment(post, state=state)
        await manager.generate_qzone_auto_comment(post, state=state)
    ...
    async def test_auto_comment_image_vision_cache_ignores_volatile_url_query(self):
        ...
        await manager.generate_qzone_auto_comment(first, state=state)
        await manager.generate_qzone_auto_comment(second, state=state)

        self.assertEqual(
            manager.context.calls,
            ["https://m.qpic.cn/psc?/V10abc/photo.jpg&token=aaa&_t=1"],
        )
    ...
    async def test_auto_comment_image_vision_cache_survives_changed_post_key(self):
        ...
        self.assertEqual(
            manager.context.calls, ["https://m.qpic.cn/psc?/V10abc/photo.jpg&token=aaa"]
        )
    ...
    async def test_auto_comment_image_vision_provider_overrides_llm_provider(self):
        ...
        class Manager(module.TaskQzoneAutoCommentService):
            def __init__(self):
                ...
                self.context = Context()
                self.plugin = types.SimpleNamespace(
                    basic_conf={"llm_provider_id": "normal-provider"},
                    context=self.context,
                )
    ...
    async def test_auto_comment_image_vision_falls_back_from_configured_to_session_provider_per_request(
        ...
        class Context:
            ...
            async def get_current_chat_provider_id(self, umo):
                return "session-provider" if umo == "session-1" else ""

            async def llm_generate(self, **kwargs):
                provider_id = kwargs["chat_provider_id"]
                self.calls.append(provider_id)
                if provider_id == "qzone-vision-provider":
                    raise RuntimeError("视觉模型暂不可用")
                return types.SimpleNamespace(
                    completion_text=f"{kwargs['image_urls'][0]} 的识别结果"
                )
        ...
        await manager.generate_qzone_auto_comment(
            first, state={}, target_umo="session-1"
        )
        await manager.generate_qzone_auto_comment(
            second, state={}, target_umo="session-1"
        )

        self.assertEqual(
            manager.context.calls,
            [
                "qzone-vision-provider",
                "session-provider",
                "qzone-vision-provider",
                "session-provider",
            ],
        )
        self.assertEqual(manager.llm_calls[0]["umo"], "session-1")

    async def test_auto_comment_image_vision_falls_back_from_session_to_system_default_provider(
        self,
    ):
        module, models = _load_auto_comment_module()
        ...
        class Context:
            ...
            async def llm_generate(self, **kwargs):
                provider_id = kwargs["chat_provider_id"]
                self.calls.append(provider_id)
                if provider_id == "session-provider":
                    raise RuntimeError("当前会话模型不支持图片")
                return types.SimpleNamespace(completion_text="默认模型识别到一只猫")
        ...
        await manager.generate_qzone_auto_comment(
            post, state={}, target_umo="session-1"
        )

        self.assertEqual(
            manager.context.calls, ["session-provider", "default-provider"]
        )
        self.assertIn("默认模型识别到一只猫", manager.prompts[0])

    async def test_auto_comment_image_vision_skips_without_configured_or_default_provider(
        self,
    ):
        module, models = _load_auto_comment_module()
    ...
    async def test_thread_reply_prompt_includes_same_floor_history(self):
        ...
        parent = models.QzoneComment(
            uin=2, nickname="测试用户甲", content="到家了吗", tid="c1", create_time=100
        )
        ...
        other_floor = models.QzoneComment(
            uin=3, nickname="路人", content="隔壁楼内容", tid="c2", create_time=250
        )
        post = models.QzonePost(
            uin=1,
            tid="self",
    ...
    async def test_thread_reply_generation_keeps_complete_short_sentence(self):
        ...
        parent = models.QzoneComment(
            uin=2, nickname="测试用户甲", content="借我膝盖", tid="c1"
        )
        target = models.QzoneComment(
            uin=2, nickname="测试用户甲", content="想靠一下", tid="r1", parent_tid="c1"
        )
    ...
    async def test_thread_reply_submit_keeps_pending_reply_complete(self):
        ...
        class Service:
            ...
            async def reply_comment(
                self, post_id, comment, content, *, parent_comment=None
            ):
                self.reply = content
                return {
        ...
        self.assertEqual(
            manager.plugin.qzone_service.reply,
            "既然是真的，那今晚的疯狂星期四就靠你表现了。",
        )
        self.assertEqual(
            processed["1:self:r1"]["content"],
            "既然是真的，那今晚的疯狂星期四就靠你表现了。",
        )

    async def test_auto_interaction_system_prompt_always_includes_persona(self):
        module, _models = _load_auto_comment_module()

        class ContentService:
            async def get_persona_info(self):
                return {"prompt": "默认系统人设。"}
        ...
        system_prompt = await manager._qzone_auto_interaction_system_prompt(
            "只输出回复正文。"
        )

        self.assertIn("只输出回复正文。", system_prompt)
        self.assertIn("必须以提示中的【当前本地时间】为准", system_prompt)
        self.assertIn("默认系统人设。", system_prompt)
    ...
    async def test_execute_comments_only_commentable_friend_posts(self):
        ...
        self.assertEqual(
            manager.plugin.qzone_service.comments, [("3:new", "这家书店听起来好舒服")]
        )
    ...
    async def test_auto_comment_active_window_skips_old_posts(self):
        module, models = _load_auto_comment_module()
        policy_module = sys.modules[f"{TASKS_PACKAGE_NAME}.interact.policy"]
        now = 2_000_000_000
        ...
        class Manager(module.TaskQzoneAutoCommentService):
            def __init__(self):
                self.qzone_conf = {
                    ...
                    "qzone_enable_auto_comment": True,
                    "qzone_auto_comment_limit": 2,
                    "qzone_auto_interaction_active_hours": 1,
                }
                self.db = FakeDb()
                self.plugin = types.SimpleNamespace(
                    _is_terminated=False,
        ...
        with (
            patch.object(policy_module.time, "time", lambda: now),
        ...
        self.assertEqual(
            manager.plugin.qzone_service.comments, [("3:new", "刚发这条挺有意思")]
        )
    ...
    async def test_auto_comment_prioritizes_mention_posts(self):
        ...
        class Service:
            ...
            async def query_mention_posts(
                self, *, offset=0, count=10, with_detail=True
            ):
    ...
    async def test_auto_comment_send_failure_waits_until_next_run(self):
        ...
        class Service:
            def __init__(self):
                ...
                self.posts = [
                    models.QzonePost(
                        uin=3, tid="new", name="朋友B", text="今天去了书店"
                    ),
                ]
    ...
    async def test_auto_comment_processed_alias_skips_changed_post_key(self):
        ...
        self.assertEqual(
            manager.plugin.qzone_service.comments, [("3:feed-key", "这张真不错")]
        )
        self.assertEqual(len(manager.prompts), 1)
        state = manager.db.state[module.QZONE_AUTO_COMMENT_STATE_KEY]
        self.assertEqual(state["processed"]["3:feed-key"]["action"], "commented")
        self.assertEqual(
            state["processed"]["post:3:311:curkey:stable-curkey"]["action"], "commented"
        )
    ...
    async def test_auto_comment_llm_skip_word_counts_generation_failure_without_processing(
        self,
    ):
        module, models = _load_auto_comment_module()
        ...
        class Service:
            def __init__(self):
                ...
                self.posts = [
                    models.QzonePost(uin=1, tid="self", name="我", text="自己的动态"),
                    models.QzonePost(
                        uin=2, tid="sad", name="朋友A", text="今天生病了好难受"
                    ),
                    models.QzonePost(
                        uin=3, tid="happy", name="朋友B", text="今天做的蛋糕成功了"
                    ),
                ]
    ...
    async def test_auto_like_active_window_skips_old_posts(self):
        module, models = _load_auto_comment_module()
        policy_module = sys.modules[f"{TASKS_PACKAGE_NAME}.interact.policy"]
        now = 2_000_000_000
        ...
        self.assertEqual(result["skipped"], 1)
        self.assertEqual(manager.plugin.qzone_service.likes, ["3:new"])

    async def test_auto_like_can_scan_target_qzone(self):
        module, models = _load_auto_comment_module()

        class Service:
            def __init__(self):
                self.likes = []
                self.query_posts_calls = []

            async def context(self):
                return types.SimpleNamespace(uin=1)

            async def query_posts(
                self, *, target_id="", pos=0, num=5, with_detail=False
            ):
                self.query_posts_calls.append(
                    {
                        "target_id": target_id,
                        "pos": pos,
                        "num": num,
            ...
            async def query_recent_posts(self, **kwargs):
                raise AssertionError(
                    "targeted like should not query global friend feeds"
                )
        ...
        with patch.object(module.asyncio, "sleep", no_sleep):
            result = await manager.execute_qzone_auto_like(target_id="10001")

        self.assertEqual(result["liked"], 1)
        self.assertEqual(manager.plugin.qzone_service.likes, ["10001:target-post"])
        self.assertEqual(
            manager.plugin.qzone_service.query_posts_calls[0]["target_id"], "10001"
        )
        self.assertTrue(
            manager.plugin.qzone_service.query_posts_calls[0]["with_detail"]
        )
    ...
    async def test_auto_like_processed_alias_skips_changed_post_key(self):
        ...
        self.assertEqual(
            state["processed"]["post:3:311:curkey:stable-curkey"]["action"], "liked"
        )
    ...
    async def test_auto_like_processed_body_alias_skips_changed_post_identity(self):
                ...
                return [
                    models.QzonePost(
                        ...
                        images=[
                            ...
                            if self.run == 1
                            else "https://example.com/gallery-b.jpg"
                        ],
                    )
                ]
        ...
        self.assertTrue(
            any(
                key.startswith("post:3:311:body:") and item.get("action") == "liked"
    ...
    async def test_auto_like_does_not_call_llm(self):
        ...
        class Service:
            def __init__(self):
                ...
                self.posts = [
                    models.QzonePost(
                        uin=3, tid="friendly", name="朋友B", text="测试互动内容"
                    ),
                ]
    ...
    async def test_auto_like_persists_success_before_task_can_be_cancelled(self):
        module, models = _load_auto_comment_module()
        ...
        with patch.object(module.asyncio, "sleep", cancelled_sleep):
            with self.assertRaises(asyncio.CancelledError):
                await manager.execute_qzone_auto_like()
    ...
    async def test_execute_replies_to_new_comments_on_self_posts(self):
                ...
                self.posts = [
                    models.QzonePost(
                        ...
                        comments=[
                            ...
                            models.QzoneComment(
                                uin=1, nickname="Me", content="自己的评论", tid="mine"
                            ),
                            models.QzoneComment(
                                uin=3, nickname="Bob", content="已回复", tid="c2"
                            ),
                            models.QzoneComment(
                                uin=1,
                                nickname="Me",
                    ...
                    models.QzonePost(
                        ...
                        comments=[
                            models.QzoneComment(
                                uin=2, nickname="Alice", content="路过", tid="c3"
                            )
                        ],
                    ),
                ]
        ...
        self.assertEqual(
            manager.plugin.qzone_service.replies, [("1:self", "c1", "谢谢你呀")]
        )
        state = manager.db.state[module.QZONE_AUTO_REPLY_STATE_KEY]
        self.assertEqual(state["processed"]["1:self:c1"]["action"], "replied")

    async def test_auto_reply_active_window_skips_old_comments(self):
        module, models = _load_auto_comment_module()
        policy_module = sys.modules[f"{TASKS_PACKAGE_NAME}.interact.policy"]
        now = 2_000_000_000
        ...
        with patch.object(policy_module.time, "time", lambda: now):
            with patch.object(module.asyncio, "sleep", no_sleep):
                result = await manager.execute_qzone_auto_reply()
        ...
        self.assertEqual(result["skipped"], 1)
        self.assertEqual(
            manager.plugin.qzone_service.replies, [("1:self", "new", "reply")]
        )
        state = manager.db.state[module.QZONE_AUTO_REPLY_STATE_KEY]
        self.assertNotIn("1:self:old", state["processed"])
        self.assertEqual(state["processed"]["1:self:new"]["action"], "replied")
    ...
    async def test_auto_reply_llm_skip_word_counts_generation_failure_without_processing(
        self,
    ):
        module, models = _load_auto_comment_module()
    ...
    async def test_execute_replies_to_second_level_reply_on_self_post(self):
        ...
        self.assertEqual(result["replied"], 1)
        self.assertEqual(
            manager.plugin.qzone_service.replies, [("1:self", "r2", "哈哈是呀")]
        )
        state = manager.db.state[module.QZONE_AUTO_REPLY_STATE_KEY]
    ...
    async def test_execute_prefers_latest_thread_reply_over_first_comment(self):
        ...
        self.assertEqual(result["replied"], 1)
        self.assertEqual(
            manager.plugin.qzone_service.replies, [("1:self", "r2", "接住了", "c2")]
        )
        state = manager.db.state[module.QZONE_AUTO_REPLY_STATE_KEY]
        self.assertEqual(state["processed"]["1:self:r2"]["action"], "thread_replied")
    ...
    async def test_execute_replies_to_parent_comment_without_prior_thread_reply(self):
        ...
        self.assertEqual(result["replied"], 1)
        self.assertEqual(
            manager.plugin.qzone_service.replies, [("1:self", "c1", "确实挺开心", "")]
        )
        state = manager.db.state[module.QZONE_AUTO_REPLY_STATE_KEY]
        self.assertEqual(state["processed"]["1:self:c1"]["action"], "replied")

    async def test_execute_replies_to_second_level_reply_when_qzone_returns_reverse_order(
        self,
    ):
        module, models = _load_auto_comment_module()
    ...
    async def test_execute_replies_to_thread_reply_with_same_timestamp_as_self_reply(
        self,
    ):
        module, models = _load_auto_comment_module()
        ...
        self.assertEqual(result["replied"], 1)
        self.assertEqual(
            manager.plugin.qzone_service.replies, [("1:self", "r4", "reply 3", "c1")]
        )
        state = manager.db.state[module.QZONE_AUTO_REPLY_STATE_KEY]
        self.assertEqual(state["processed"]["1:self:r4"]["action"], "thread_replied")

    async def test_execute_does_not_mark_thread_replied_when_reply_lands_on_wrong_floor(
        self,
    ):
        module, models = _load_auto_comment_module()
        ...
        class Service:
            ...
            async def reply_comment(
                ...
                setattr(
                    exc,
                    "attempted_targets",
                    [{"comment_id": comment.tid, "comment_uin": comment.uin}],
                )
        ...
        self.assertEqual(result["failed"], 0)
        self.assertEqual(
            manager.plugin.qzone_service.replies, [("1:self", "r2", "reply 2", "c1")]
        )
        state = manager.db.state[module.QZONE_AUTO_REPLY_STATE_KEY]
        ...
        self.assertTrue(
            any("QQ 空间自动回评落点校验未通过" in message for message in logs)
        )
                    ...
                    models.QzonePost(
                        ...
                        comments=[
                            ...
                            models.QzoneComment(
                                ...
                                raw_tid="2",
                                parent_tid="6",
                                reply_to_tid="unknown_synthetic_target",
                                raw_reply_to_tid="1",
                                reply_to_uin=1,
                                raw_reply_to_uin=1,
                                create_time=300,
    ...
    async def test_execute_replies_to_new_thread_reply_with_reused_short_id(self):
        ...
        class Service:
            ...
            async def reply_comment(
                ...
                self.replies.append(
                    (post_id, comment.tid, getattr(comment, "submit_tid", ""), content)
                )
                return {
                    "comment_id": str(
                        getattr(comment, "submit_tid", "")
                        or getattr(comment, "tid", "")
                        or ""
                    ),
                    "comment_uin": int(getattr(comment, "uin", 0) or 0),
                    "transport": "pc",
        ...
        self.assertEqual(result["replied"], 1)
        self.assertEqual(
            manager.plugin.qzone_service.replies,
            [("1:self", "11_r_1_2", "1", "就你懂得捧场")],
        )
        state = manager.db.state[module.QZONE_AUTO_REPLY_STATE_KEY]
        self.assertEqual(
            state["processed"]["1:self:11_r_1_2"]["action"], "thread_replied"
        )
        self.assertEqual(
            state["processed"]["1:self:11_r_1_2"]["submitted_comment_id"], "1"
        )
        self.assertEqual(
            state["processed"]["1:self:11_r_1_2"]["submitted_transport"], "pc"
        )
        self.assertEqual(
    ...
    async def test_execute_replies_to_top_level_comment_with_post_parent_tid(self):
        ...
        self.assertEqual(result["replied"], 1)
        self.assertEqual(
            manager.plugin.qzone_service.replies, [("1:self", "c1", "reply", None)]
        )
    ...
    async def test_execute_auto_reply_uses_merged_thread_reply_after_detail_merge(self):
        ...
        self.assertEqual(result["replied"], 1)
        self.assertEqual(
            manager.plugin.qzone_service.replies, [("1:self", "r2", "哈哈是呀", "c1")]
        )
    ...
    async def test_execute_auto_comment_continues_friend_thread_under_bot_comment(self):
        ...
        self.assertEqual(
            manager.plugin.qzone_service.replies,
            [("2:friend", "r1", "行，下次一起去", "c1")],
        )
        state = manager.db.state[module.QZONE_AUTO_COMMENT_STATE_KEY]
        self.assertEqual(
            state["processed"]["2:friend:r1"]["action"], "thread_commented"
        )
        self.assertEqual(state["processed"]["2:friend:r1"]["parent_comment_id"], "c1")

    async def test_auto_comment_friend_thread_active_window_skips_old_replies(self):
        module, models = _load_auto_comment_module()
        policy_module = sys.modules[f"{TASKS_PACKAGE_NAME}.interact.policy"]
        now = 2_000_000_000
        ...
        self.assertEqual(manager.plugin.qzone_service.top_comments, [])
        self.assertEqual(
            manager.plugin.qzone_service.replies, [("2:friend", "new", "reply", "c1")]
        )
        state = manager.db.state[module.QZONE_AUTO_COMMENT_STATE_KEY]
        self.assertNotIn("2:friend:old", state["processed"])
        self.assertEqual(
            state["processed"]["2:friend:new"]["action"], "thread_commented"
        )

    async def test_execute_auto_comment_does_not_top_comment_same_post_after_thread_reply(
        self,
    ):
        module, models = _load_auto_comment_module()
    ...
    async def test_execute_auto_comment_merges_stale_mention_with_recent_detail(self):
        ...
        class Service:
            ...
            async def query_mention_posts(
                self, *, offset=0, count=5, with_detail=False
            ):
        ...
        self.assertEqual(
            service.replies, [("2:friend", "r1", "有，艺术加工那种码", "c1")]
        )
        self.assertEqual(len(manager.prompts), 1)
        self.assertNotIn("【配图识别】", manager.prompts[0])
    ...
    async def test_execute_auto_comment_continues_latest_friend_thread_reply(self):
        ...
        state = manager.db.state[module.QZONE_AUTO_COMMENT_STATE_KEY]
        self.assertEqual(
            state["processed"]["2:friend:r2"]["action"], "thread_commented"
        )

    async def test_execute_auto_comment_continues_friend_thread_with_colliding_short_id(
        self,
    ):
        module, models = _load_auto_comment_module()
        ...
        class Service:
            ...
            async def comment(self, post_id, content):
                raise AssertionError(
                    "thread continuation should not post a top-level comment"
                )
            ...
            def _filter_thread_reply_targets(
                self, post, comment, *, parent_comment=None, targets=None
            ):
            ...
            def _thread_reply_payload_variants(
                self, post, comment, parent_comment, targets
            ):
                return service_module.QzoneService._thread_reply_payload_variants(
                    post, comment, parent_comment, targets
                )

            def unsafe_thread_reply_target_reason(
                self, comment, *, parent_comment=None
            ):
            ...
            def has_thread_reply_submit_plan(
                self, post, comment, *, parent_comment=None
            ):
                return service_module.QzoneService.has_thread_reply_submit_plan(
                    self,
                    post,
                    comment,
                    parent_comment=parent_comment,
        ...
        state = manager.db.state[module.QZONE_AUTO_COMMENT_STATE_KEY]
        self.assertEqual(
            state["processed"]["100000002:dca659054aad3c6a8d8e0600:1_r_1_100000002"][
                "action"
            ],
            "thread_commented",
        )

    async def test_execute_auto_comment_skips_friend_post_with_self_comment_but_no_new_thread_reply(
        self,
    ):
        module, models = _load_auto_comment_module()
        ...
        class Manager(module.TaskQzoneAutoCommentService):
            ...
            async def llm(self, **kwargs):
                raise AssertionError(
                    "friend post with existing self comment should not ask LLM"
                )
    ...
    async def test_execute_auto_reply_does_not_scan_friend_sources(self):
        ...
        class Service:
            ...
            async def query_about_me(self, *, offset=0, count=10):
                self.about_calls += 1
                raise AssertionError(
                    "auto reply should not query about-me friend source"
                )
    ...
    async def test_auto_comment_rate_limit_records_retry_without_warning_failure(self):
        ...
        with patch.object(
            discuss_module.logger,
            "debug",
            lambda message, *args, **kwargs: logs.append(("debug", str(message))),
        ):
            with patch.object(
                discuss_module.logger,
                "warning",
                lambda message, *args, **kwargs: logs.append(("warning", str(message))),
            ):
                result = await manager.execute_qzone_auto_comment()
    ...
    async def test_execute_auto_interaction_aggregates_comment_and_reply_results(self):
        ...
        self.assertEqual(
            manager.calls, [("like", False), ("comment", False), ("reply", False)]
        )
