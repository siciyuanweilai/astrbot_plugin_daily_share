...
class DashboardMediaPageService(PanelComponent):
    ...
    async def _page_media_page(
        ...
        if days is None:
            days = self.media_page._page_dashboard_dynamic_days()
        media_kind = self.media_kind._page_dynamic_media_kind(media_kind)
        share_type = self.media_kind._page_dynamic_share_type(share_type)
        ...
        return {
            "items": await self.media_preview._page_prepare_media_items(rows[:limit]),
            "limit": limit,
    ...
    async def page_media(self):
        async def handler():
            ...
            return {
                "ok": True,
                "data": await self.media_page._page_media_page(
                    limit,
    ...
    async def page_media_delete(self):
        async def handler():
            ...
            ids = self.media_page._page_history_ids_from_body(body)
            if not ids:
            ...
            if deleted > 0:
                file_delete = await self.media_files._page_delete_local_media_files(
                    history_items
                )
            media_page = await self.media_page._page_media_page(
                limit,
                media_kind=media_kind,
                share_type=share_type,
            )
    ...
    async def page_media_view(self):
        async def handler():
            ...
            if history_id is None:
                raise RuntimeError("缺少历史记录 ID")
            history_id = int(history_id)

            item = await self.db.get_history_by_id(history_id)
            if not item:
                raise FileNotFoundError("未找到媒体记录")
            if self.media_kind._page_media_kind(item) != "image":
            ...
            return {
                ...
                "data": {
                    "id": item.get("id"),
                    "media_type": "image",
                    **await self.media_preview._page_view_image_payload(
                        item, history_id
                    ),
                },
            }

        return await self.server._page_json(
            handler, self.server._page_media_cache_headers()
        )
