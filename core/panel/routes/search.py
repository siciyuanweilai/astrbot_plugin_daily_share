...
class DashboardRouteQueryService(PanelComponent):
    async def page_history(self):
        async def handler():
            ...
            return {
                "ok": True,
                "data": {
                    "items": await self.labels._page_prepare_history_items(history)
                },
            }
    ...
    async def page_failures(self):
        async def handler():
            ...
            return {
                "ok": True,
                "data": {
                    "items": await self.labels._page_prepare_history_items(
