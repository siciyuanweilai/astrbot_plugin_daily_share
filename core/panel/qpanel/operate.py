...
class DashboardQzoneActionService(PanelComponent):
    async def page_qzone_like(self):
        async def handler():
            body = await self.server._page_json_body()
            post_id = self.qzone_actions._page_qzone_required_post_id(body)
            await self.qzone_service.like(post_id)
            self.events.emit_dashboard_event(
                "qzone", {"action": "like", "post_id": post_id}
            )
    ...
    async def page_qzone_comment(self):
        async def handler():
            ...
            self.events.emit_dashboard_event(
                "qzone", {"action": "comment", "post_id": post_id}
            )
    ...
    async def page_qzone_delete(self):
        async def handler():
            ...
            self.events.emit_dashboard_event(
                "qzone", {"action": "delete", "post_id": post_id}
            )
