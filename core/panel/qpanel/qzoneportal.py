...
class DashboardQzoneEntryService(PanelComponent):
    async def page_qzone_entry(self):
        async def handler():
            params = await self.server._page_query_params()
            entry = str(params.get("entry") or "").strip().lower()
            pos, num = self.qzone_tools._page_qzone_page_args(params)
            ctx = await self.qzone_service.context()

            if entry == "about":
                payload = await self.qzone_entry._page_qzone_about_payload(
                    ctx, pos=pos, num=num
                )
            elif entry == "today":
                payload = await self.qzone_entry._page_qzone_today_payload(
                    ctx, params=params, pos=pos, num=num
                )
            elif entry == "board":
                payload = await self.qzone_entry._page_qzone_board_payload(
                    params, pos=pos, num=num
                )
            else:
    ...
    async def _page_qzone_about_payload(self, ctx, *, pos: int, num: int) -> dict:
        ...
        items = self.qzone_tools._page_qzone_post_items(
            result.get("items") or [], self_uin=ctx.uin, include_comments=True
        )
        return {
            "kind": "posts",
            "items": items,
    ...
    async def _page_qzone_today_payload(
        self, ctx, *, params: dict, pos: int, num: int
    ) -> dict:
        try:
    ...
    async def _page_qzone_board_payload(
        self, params: dict, *, pos: int, num: int
    ) -> dict:
        target_id = str(params.get("target_id") or "").strip()
        result = await self.qzone_service.query_message_board(
            target_id=target_id, start=pos, num=num
        )
