...
class DashboardQzoneFeedService(PanelComponent):
    async def page_qzone_feed(self):
        async def handler():
            ...
            with_detail = str(params.get("detail") or "").lower() in {
                "1",
                "true",
                "yes",
                "on",
            ...
            try:
                ...
                logger.debug(f"[日常分享] QQ 空间面板登录态暂不可用: {exc}")
                return {
                    "ok": True,
                    "data": {
            ...
            try:
                ...
                logger.debug(f"[日常分享] QQ 空间面板说说暂不可用: {exc}")
                posts = []
                has_more = False
            ...
            return {
                ...
                "data": {
                    ...
                    "items": self.qzone_tools._page_qzone_post_items(
                        posts, self_uin=ctx.uin, include_comments=True
                    ),
                    "pos": pos,
                    "num": num,
                    "has_more": has_more,
