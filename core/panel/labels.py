...
class DashboardLabelsService(PanelComponent):
    ...
    def _page_target_kind_map(self, targets: dict) -> dict:
        ...
        for bucket, kind in bucket_kinds.items():
            for item in (targets or {}).get(bucket, []):
                ...
                keys = [target_id]
                try:
                    _adapter_id, real_id = self.ctx_service.parse_umo(target_id)
                except Exception:
                    real_id = ""
                if real_id:
                    keys.append(real_id)
    ...
    def _page_target_label(self, target_id: str) -> str:
        ...
        try:
            ...
            logger.debug(f"[日常分享] 构建仪表盘目标显示名失败: {raw}, {exc}")
            return ""

    def _page_target_probe(self, target_id: str, kind: str = "") -> tuple:
        raw = str(target_id or "").strip()
        try:
            adapter_id, real_id = self.ctx_service.parse_umo(raw)
        except Exception as exc:
            logger.debug(f"[日常分享] 解析仪表盘目标失败: {raw}, {exc}")
            adapter_id, real_id = "", ""
    ...
    def _get_page_target_label_cache(self) -> dict:
        cache = self.__dict__.get("_page_target_label_cache_data")
    ...
    def _page_target_label_cache_keys(self, target_id: str, kind: str = "") -> list:
        raw, _adapter_id, probe_id, _is_group = self.labels._page_target_probe(
            target_id, kind
        )
    ...
    async def _fetch_page_target_label(self, target_id: str, kind: str = "") -> str:
        raw, adapter_id, probe_id, is_group = self.labels._page_target_probe(
            target_id, kind
        )
        ...
        try:
            bot = self.ctx_service.get_onebot_bot(raw, adapter_id=adapter_id)
        except Exception as exc:
            logger.debug(f"[日常分享] 查找仪表盘 QQ 机器人客户端失败: {raw}, {exc}")
            return ""
        if not bot:
            return ""
        ...
        try:
            if is_group:
                result = await self.ctx_service.call_onebot_action(
                    bot,
                    "get_group_info",
            ...
            logger.debug(f"[日常分享] 解析仪表盘目标显示名失败: {raw}, {exc}")
        return ""

    async def _resolve_page_target_label(self, target_id: str, kind: str = "") -> str:
        label = self.labels._page_target_label(target_id)
        if label:
            return label

        cache = self.labels._get_page_target_label_cache()
        keys = self.labels._page_target_label_cache_keys(target_id, kind)
        for key in keys:
            label = str(cache.get(key) or "").strip()
            if label:
                return label

        label = await self.labels._fetch_page_target_label(target_id, kind)
        if label:
            for key in keys:
                cache[key] = label
        return label
    ...
    async def _page_prepare_history_items(self, items: list) -> list:
        ...
        for item in items:
            item = dict(item)
            label = await self.labels._resolve_page_target_label(
                item.get("target_id", "")
            )
            if label:
