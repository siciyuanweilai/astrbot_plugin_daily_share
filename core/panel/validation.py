...
from ..config import CRON_TEMPLATES, NEWS_SOURCE_MAP, ShareType
from ..constants import (
    SOURCE_CN_MAP,
    canonical_share_type_token,
    normalize_share_type_token,
)
from .common import (
    _PAGE_CLOCK_TIME_RE,
...
class DashboardConfigValidationService(PanelComponent):
    ...
    def _page_share_type(self, value):
        raw = str(value or "自动").strip()
        if not raw or raw == "自动":
            return None
        normalized = normalize_share_type_token(raw)
        if normalized:
            return ShareType(normalized)
        raise RuntimeError(f"不支持的分享类型: {raw}")
    ...
    def _page_list_value(
        ...
        *,
        ...
        return DashboardConfigValidationService._page_ordered_list_value(
            value,
            max_items=max_items,
    ...
    def _page_ordered_list_value(
        ...
        *,
        ...
        raw_items = DashboardConfigValidationService._page_raw_list_items(
            value, split_commas=split_commas
        )
        result = []
        seen = set()
    ...
    @staticmethod
    def _page_raw_list_items(value: Any, *, split_commas: bool) -> list:
    ...
    def _page_choice_value(
        self, value: Any, allowed: set, default: str, label: str
    ) -> str:
        raw = self.validation._page_clean_text(
            value if value not in (None, "") else default, max_len=80
        )
    ...
    def _page_cron_value(self, value: Any, default: str, label: str) -> str:
        ...
        if self.task_manager.schedule.parse_cron_to_kwargs(actual) is None:
            raise RuntimeError(f"{label} 无效，需填写预设或标准 5 位定时表达式: {raw}")
        return raw

    def _page_fixed_times_value(self, value: Any, default: list, label: str) -> list:
        items = self.validation._page_list_value(
        ...
        for item in items:
            if not _PAGE_CLOCK_TIME_RE.match(item):
                raise RuntimeError(f"{label} 时间格式无效: {item}")
        return items
    ...
    def _page_quiet_hours_value(self, value: Any, label: str) -> list:
        items = self.validation._page_list_value(
            value,
            max_items=24,
            item_max_len=32,
        ...
        for item in items:
            ...
            start, end = item.split("-", 1)
            if start == end:
                raise RuntimeError(f"{label} 开始和结束时间不能相同: {item}")
        return items

    def _page_type_list_value(self, value: Any, label: str) -> list:
        allowed = _PAGE_SHARE_TYPE_OPTIONS - {"自动"}
        raw_items = self.validation._page_list_value(
            value, max_items=len(allowed), item_max_len=32, split_commas=True
        )
        items = [canonical_share_type_token(item) for item in raw_items]
        invalid = [
            item
            for item, normalized in zip(raw_items, items)
    ...
    def _page_sequence_value(self, value: Any, default: list, label: str) -> list:
        allowed = _PAGE_SHARE_TYPE_OPTIONS - {"自动"}
        raw_items = self.validation._page_ordered_list_value(
            value if value not in (None, "") else default,
            max_items=24,
            item_max_len=32,
            split_commas=True,
    ...
    def _page_news_source_list_value(self, value: Any, label: str) -> list:
        items = self.validation._page_list_value(
            value, max_items=len(NEWS_SOURCE_MAP), item_max_len=40, split_commas=True
        )
        normalized = []
        invalid = []
        for item in items:
            source = item if item in NEWS_SOURCE_MAP else SOURCE_CN_MAP.get(item)
    ...
    def _page_contact_aliases_value(self, value: Any) -> list:
        items = self.validation._page_list_value(value, max_items=300, item_max_len=300)
        result = []
        for item in items:
            ...
            if ":" not in normalized:
                raise RuntimeError("用户称呼映射格式应为用户标识/会话标识:昵称")
    ...
    def _page_delay_range_value(self, value: Any, default: str = "1.0-2.0") -> str:
        raw = self.validation._page_clean_text(
            value if value not in (None, "") else default, max_len=32
        )
        parts = raw.split("-", 1)
        try:
            if len(parts) == 2:
