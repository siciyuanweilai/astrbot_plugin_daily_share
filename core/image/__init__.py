...
from .motion import ImageVideoMixin
from .composer import ImageVisualMixin
...
class ImageService(ImageVisualMixin, ImageVideoMixin):
    ...
    async def generate_image(
        ...
        # 1. 智能判断：是否画人
        involves_self = await self._check_involves_self(content, share_type, target_umo=target_umo)
        ...
        logger.info(f"[每日分享] 配图决策: {mode_str} ({logic_str}) | 类型: {share_type_label(share_type)}")
        
        # 3. 智能提取视觉元素
        visuals = {}
        if content or life_context:
            visuals = await self._agent_extract_visuals(
                content,
                life_context,
        ...
        # 4. 组装最终提示词
        prompt = await self._assemble_final_prompt(content, share_type, involves_self, visuals, target_umo=target_umo)
