...
from .chronicle import ContextHistoryMixin
...
class ContextService(
    ...
    def _parse_umo(self, target_umo: str):
        ...
        try:
            ...
            logger.debug(f"[每日分享] 解析运行时会话标识失败: {e}")
            return None, None
        ...
        try:
            ...
            for platform in iter_platform_instances(self.context):
                ...
                if bot_client and p_id:
                    ...
                    count += 1
                    logger.debug(f"[每日分享] 发现并缓存机器人实例: {p_id}，实例类型: {type(bot_client).__name__}")
            
            logger.debug(f"[每日分享] 机器人缓存初始化完成，共发现 {count} 个实例。")
            
        except Exception as e:
            logger.error(f"[每日分享] 机器人初始化失败: {e}")
