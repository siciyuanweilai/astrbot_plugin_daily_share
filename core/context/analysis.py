from .shared import Any, Dict, List, ShareType, datetime, logger, time
from .shared import DAILY_SHARE_SOURCE
...
class ContextHistoryAnalysisMixin:
    ...
    def format_history_prompt(self, history_data: Dict, share_type: ShareType) -> str:
        """格式化提示词。"""
        if not history_data or not history_data.get("messages"): return ""
        is_group = history_data.get("is_group", False)
        messages = history_data["messages"]
        if is_group:
            return self._format_group_chat_for_prompt(messages, history_data.get("group_info", {}), share_type)
        else:
            return self._format_private_chat_for_prompt(messages, share_type)

    def _format_group_chat_for_prompt(self, messages: List[Dict], group_info: Dict, share_type: ShareType) -> str:
        ...
        elif share_type == ShareType.NEWS: hint = "选择可能引起群内讨论的新闻"
        elif share_type == ShareType.MOOD: hint = "可以简单分享心情，但不要过于私人"
        else: hint = ""
    ...
    def _format_private_chat_for_prompt(self, messages: List[Dict], share_type: ShareType) -> str:
        max_length = 500
        if share_type == ShareType.GREETING: hint = "可以根据最近的对话内容打招呼"
        elif share_type == ShareType.MOOD: hint = "可以延续最近的话题或感受"
        elif share_type == ShareType.NEWS: hint = "可以根据对方的兴趣选择新闻"
        ...
        for m in reversed(messages[-5:]):
            ...
            if m.get("source") == DAILY_SHARE_SOURCE:
