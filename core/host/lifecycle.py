...
class PluginRuntimeMixin:
    ...
    def _track_task(self, coro):
        ...
        try:
            ...
            coro.close()
            log_exception("[每日分享] 创建后台任务失败", exc, level="warning", with_traceback=False)
            return None
        ...
        def _cleanup(done_task):
            ...
            try:
                ...
                return
            if exc:
                log_exception("[每日分享] 后台任务异常", exc)

        task.add_done_callback(_cleanup)
        return task

    async def _cancel_background_tasks(self, *, timeout: float = 5.0) -> None:
        tasks = [task for task in list(self._bg_tasks) if task and not task.done()]
        if not tasks:
        ...
        try:
            ...
            logger.warning(f"[每日分享] 后台任务取消超时，仍有 {len(self._bg_tasks)} 个任务未结束")
        finally:
            self._bg_tasks.difference_update(task for task in tasks if task.done())
    ...
    async def _ensure_share_command_outer_permission(self) -> None:
        ...
        try:
            ...
            log_exception("[每日分享] 跳过命令外层权限修正", exc, level="debug", with_traceback=False)
            return
        ...
        try:
            ...
            log_exception("[每日分享] 修正 /分享 外层命令权限失败", exc, level="warning", with_traceback=False)
    ...
    async def _delayed_init(self):
        ...
        try:
            ...
            log_exception("[每日分享] 启动清理过期数据失败", e, level="warning", with_traceback=False)
    ...
    async def _delayed_init_bots(self):
        ...
        try:
            ...
            log_exception("[每日分享] 机器人初始化任务出错", e)
