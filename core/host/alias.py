...
class PluginAliasService(SupportComponent):
    ...
    def _target_alias_keys(
        ...
        def append_real_id(value: str) -> None:
            try:
                _, real_id = self.ctx_service.parse_umo(value)
                if real_id:
                    keys.append(real_id)
            except Exception as e:
                logger.debug(f"[日常分享] 解析昵称映射目标失败: {e}")
        ...
        if event:
            ...
            try:
                ...
                logger.debug(f"[日常分享] 读取发送者标识失败: {e}")
        return list(dict.fromkeys(k for k in keys if k))

    def get_contact_alias(
        self, target_uid: str, event: AstrMessageEvent | None = None
    ) -> str:
        aliases = self.aliases._normalize_contact_aliases()
        for key in self.aliases._target_alias_keys(target_uid, event):
    ...
    def set_contact_alias(
        ...
        aliases = self.aliases._normalize_contact_aliases()
        keys = self.aliases._target_alias_keys(target_uid, event)
        save_key = ""
        for key in keys:
            if not self.task_manager.targets.is_full_umo(key):
                save_key = key
                break
        if not save_key and keys:
            _, real_id = self.ctx_service.parse_umo(keys[0])
            save_key = real_id or keys[0]
        if not save_key:
            return ""
        aliases[save_key] = str(alias or "").strip()
        serialized_aliases = self.aliases._serialize_contact_aliases(aliases)
