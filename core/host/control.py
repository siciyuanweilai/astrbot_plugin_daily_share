...
class PluginManualShareService(SupportComponent):
    @staticmethod
    ...
    def _manual_share_scope(
        self, current_uid: str, *, is_broadcast: bool, is_qzone_target: bool
    ) -> tuple[str | None, bool]:
        return (None if is_broadcast else current_uid), bool(
            is_broadcast or is_qzone_target
        )
    ...
    async def _check_share_command_permission(
        ...
        *,
        ...
        if arg in admin_only_args or is_broadcast or is_qzone_target:
            if not is_admin:
                return False, event.plain_result(
                    "权限不足：该操作会修改全局配置、广播或发布QQ空间，仅管理员可用。"
                )
        elif not (is_admin or is_configured_receiver):
            return False, self.permissions._plain_permission_denied(event)
        return True, None

    async def _route_share_config_command(
        self, event: AstrMessageEvent, arg: str, parts: list[str]
    ):
        routes = {
            "早报空间": lambda: self.command_handler.cmd_briefing_qzone_sync(
                event, parts
            ),
    ...
    def _parse_share_command_parts(
