...
class PluginShareStartRouteService(SupportComponent):
    async def _start_news_image_task(
        ...
        *,
        ...
        started = await self.jobs._start_manual_share_task(
            ...
            task_factory=lambda: self.news_outbox._run_news_image_share(
                event,
                news_src=news_src,
    ...
    async def _start_typed_share_task(
        ...
        *,
        ...
        if self.is_share_busy(specific_target, global_scope=share_global_scope):
            yield event.plain_result("正如火如荼地准备中，请稍后...")
            return
        ...
        if is_qzone_target:
            if news_source is None:
                ...
                def task_factory():
                    return self.task_manager.qzone_share.execute_qzone_share(
                        force_type, event=event
                    )
            else:
            ...
            def task_factory():
                return self.task_manager.share.execute_share(
                    force_type,
                    specific_target=specific_target,
                    event=event,
                )
