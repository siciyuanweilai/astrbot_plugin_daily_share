...
class PluginShareTypedRouteService(SupportComponent):
        ...
        *,
        ...
        if invalid_args:
            yield event.plain_result(
                f"无效参数: {' '.join(invalid_args)}。非新闻类型仅支持后缀：广播、空间。"
            )
            return

        async for result in self.start_route._start_typed_share_task(
            event,
            force_type=None,
            news_source=None,
            start_text="正在向QQ空间生成并分享内容(自动类型)..."
            if is_qzone_target
            else f"正在向{self.manual._manual_share_target_desc(is_broadcast=is_broadcast, is_qzone_target=False)}生成并分享内容(自动类型)...",
    ...
    async def _handle_manual_news_share_command(
        ...
        *,
        ...
        async for result in self.start_route._start_typed_share_task(
            ...
            if is_qzone_target
            else f"正在向{self.manual._manual_share_target_desc(is_broadcast=is_broadcast, is_qzone_target=False)}生成并分享{type_cn}{src_info} ...",
            is_qzone_target=is_qzone_target,
            specific_target=specific_target,
            share_global_scope=share_global_scope,
        ):
    ...
    async def _handle_manual_typed_share_command(
        ...
        *,
        ...
        async for result in self.start_route._start_typed_share_task(
            ...
            if is_qzone_target
            else f"正在向{self.manual._manual_share_target_desc(is_broadcast=is_broadcast, is_qzone_target=False)}生成并分享{type_cn} ...",
            is_qzone_target=is_qzone_target,
            specific_target=specific_target,
            share_global_scope=share_global_scope,
        ):
    ...
    async def _dispatch_manual_typed_command(
        ...
        *,
        ...
        force_type = self.manual._resolve_manual_share_type(arg)
        if not force_type:
            yield event.plain_result(
        ...
        if force_type == ShareType.NEWS:
            async for result in self.typed_route._handle_manual_news_share_command(
                event,
                parts=parts,
                current_uid=current_uid,
                force_type=force_type,
        ...
        async for result in self.typed_route._handle_manual_typed_share_command(
            event,
            parts=parts,
            force_type=force_type,
            type_cn=type_cn,
