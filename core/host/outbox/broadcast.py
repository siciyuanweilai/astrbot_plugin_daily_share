...
class ImageDeliveryShareService(SupportComponent):
    async def _share_static_image_to_qzone(
        ...
        *,
        ...
        try:
            await self.qzone.publish_qzone(text=history_text, images=[url])
            ...
            await self.db.add_sent_history(
                ...
                **self.task_manager.executor_helpers.image_history_kwargs(url),
            )
        ...
        *,
        ...
        if local_path:
            ...
            await self.db.add_sent_history(
                ...
                True,
                source_type=SOURCE_COMMAND,
                **self.task_manager.executor_helpers.image_history_kwargs(local_path),
            )
            return
        ...
        *,
        ...
        if emit_start:
            yield event.plain_result(
                f"正在向配置的所有群聊和私聊分享{broadcast_name}..."
            )
        filename = self.task_manager.delivery_assets.build_news_image_filename(
            url, broadcast_name
        )
        ...
        success_count = 0
        fail_count = 0
        for target in self.task_manager.targets.get_broadcast_targets():
            try:
                prepared_path = (
                    await self.task_manager.weixin_delivery.prepare_image_for_target(
                        target, local_path
                    )
                )
                await self.task_manager.delivery.send_message_chain(
                    target,
                ...
                fail_count += 1
                logger.error(f"[日常分享] 分享{broadcast_name}到 {target} 失败: {exc}")
                await self.db.add_sent_history(
                    target,
                    HISTORY_SHARE_BRIEFING,
        ...
        yield event.plain_result(
            f"{broadcast_name}广播完成：成功 {success_count} 个，失败 {fail_count} 个。"
        )
