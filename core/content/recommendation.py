...
class ContentRecommendationService(ContentComponent):
    async def _fetch_content_reference(
        self,
        keyword: str,
        *,
        search_kind: str,
        ...
        lines = [f"\n\n【{heading}】"]
        if info:
            lines.append(f"{baike_label}：{info}")
        if web_info:
            lines.append(f"{web_label}：{web_info}")
        return "\n".join(lines) + "\n"
    ...
    async def _gen_rec(self, ctx: dict):
        ...
        baike_context = await self._fetch_content_reference(
            ...
            heading="资料简介（真实数据，请参考它来推荐，不要自行捏造）",
            baike_label="百度百科简介",
            web_label="全网评价与亮点",
        )
        if not baike_context:
            logger.warning(
                f"[内容服务] 未命中任何外部资料，取消推荐分享: {target_work}"
            )
            return None
        logger.info(f"[内容服务] 推荐资料获取成功: {target_work}")
        ...
        opening_guide = self._recommendation_opening_guide(is_qzone, is_group)
...
"""
    ...
    @staticmethod
    def _recommendation_opening_guide(is_qzone: bool, is_group: bool) -> str:
        if is_qzone:
            return "表达自己最近喜欢、重温或发现这个作品的理由，不要写成“推荐给你/大家”的口吻。"
        if is_group:
            return "可以自然地推荐给群友，避免营销号式安利。"
        return "可以面向当前私聊对象推荐，语气像认真分享给一个朋友。"
