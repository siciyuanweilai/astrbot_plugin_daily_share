...
def _balanced_object_candidate(text: str, start: int) -> str:
    depth = 0
    quote = ""
    escape = False
...
def _json_object_candidates(raw: str) -> list[str]:
    ...
    outer_end = text.rfind("}")
    if 0 <= outer_start < outer_end:
        candidates.append(text[outer_start : outer_end + 1])

    for start_match in re.finditer(r"{", text):
        candidate = _balanced_object_candidate(text, start_match.start())
        if candidate:
...
def parse_qzone_response(text: str) -> dict[str, Any]:
    ...
    for body in candidates:
        try:
            ...
            continue
    logger.debug(f"[日常分享] QQ 空间响应解析失败: {'; '.join(parser_errors[-6:])}")
...
def _mapping_first_from_paths(
    mapping: dict[str, Any], paths: tuple[tuple[str, ...], ...]
) -> Any:
    for path in paths:
