...
def _feed_data_attr(attrs: dict[str, str], name: str) -> str:
    return _decode_js_escaped_text(
        attrs.get(f"data-{name}") or attrs.get(name) or ""
    ).strip()
...
def _recent_item_fid(
    item: dict[str, Any], common: dict[str, Any], raw_html: str
) -> str:
...
def _recent_item_uin(
    item: dict[str, Any], raw_html: str, common: dict[str, Any] | None = None
) -> int:
    common = common or {}
    candidates = (
...
def _strip_feed_author_prefix(text: str, author: str = "") -> str:
    ...
    for sep in ("：", ":"):
        ...
        if (
            ...
            or prefix == author
            or "回复" in prefix
            or re.search(r"[\u4e00-\u9fffA-Za-z0-9_ -]+$", prefix)
        ):
...
def _feed_html_blocks(markup: Any) -> list[str]:
    ...
    for index, match in enumerate(matches):
        ...
        if start < 0:
            ...
            else min(len(source), match.start() + 12000)
        )
    ...
    for mapping in _walk_mappings(payload):
        for value in mapping.values():
            if not isinstance(value, str):
                continue
            if "feed_data" not in value and "feed_data" not in _decode_js_escaped_text(
                value[:2000]
            ):
                continue
    ...
    for item in items + _recent_payload_html_items(payload):
        ...
        key = "|".join(
            ...
            for part in (
                item.get("fid")
                or item.get("tid")
                or _feed_data_attr(attrs, "fkey")
                or _feed_data_attr(attrs, "tid"),
