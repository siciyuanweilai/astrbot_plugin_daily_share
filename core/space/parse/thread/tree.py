...
class _CommentTreeBuilder:
    def __init__(self) -> None:
        self.result: list[QzoneComment] = []
        self.seen_short_tid_count: dict[tuple[str, str, int], int] = {}
        self.seen_short_tid_target: dict[tuple[str, str, int], str] = {}

    def stable_child_tid(self, tid: str, parent_tid: str = "", uin: int = 0) -> str:
        ...
        count = self.seen_short_tid_count.get(key, 0) + 1
        self.seen_short_tid_count[key] = count
        base = f"{parent_tid}_r_{tid}_{uin}" if uin else f"{parent_tid}_r_{tid}"
        return base if count <= 1 else f"{base}_n{count}"

    def append_item(
        self, item: dict[str, Any], *, parent_tid: str = "", reply_to_tid: str = ""
    ) -> None:
        ...
        comment.tid = self.stable_child_tid(raw_tid, parent_tid, comment.uin)
        if parent_tid and raw_reply_to_tid.isdigit() and reply_to_uin:
            comment.reply_to_tid = self.seen_short_tid_target.get(
                (parent_tid, raw_reply_to_tid, reply_to_uin),
                raw_reply_to_tid,
            )
        if parent_tid and raw_tid.isdigit() and comment.uin:
            self.seen_short_tid_target[(parent_tid, raw_tid, int(comment.uin or 0))] = (
                comment.tid
            )
        ...
        for child in _comment_children(item):
            self.append_item(
                child, parent_tid=child_parent_tid, reply_to_tid=child_reply_to_tid
            )


def parse_comments(items: list[dict[str, Any]]) -> list[QzoneComment]:
    builder = _CommentTreeBuilder()
