...
from ..asset import _extract_payload_image_sources
...
def _comment_param(item: dict[str, Any], name: str) -> str:
    ...
    if not text:
        return ""
    match = re.search(
        rf"(?:^|[?&\"'\s]){re.escape(name)}=([^&\"'\s<>]+)", html.unescape(text), re.I
    )
...
def _comment_images(item: dict[str, Any]) -> list[str]:
    payloads = [item]
    for key in ("pic", "rich_info", "richInfo", "originalcontentlist", "contentlist"):
        value = item.get(key)
        if value:
            payloads.append(value)
    return _extract_payload_image_sources(*payloads)
...
def _comment_from_raw(item: dict[str, Any], *, parent_tid: str = "") -> QzoneComment:
    ...
    reply_to_uin_text, _reply_to_uin_source = _first_named_text(
        item, *COMMENT_REPLY_UIN_KEYS
    )
    if not reply_to_uin_text:
        reply_to_uin_text = _comment_param(item, "t2_uin") or _comment_mention_param(
            item, "uin"
        )
    reply_to_uin = _safe_int(reply_to_uin_text)
    reply_to_nickname = _first_text(
        _mapping_first(item, COMMENT_REPLY_NICKNAME_KEYS),
        _comment_mention_param(item, "nick"),
    )
    return QzoneComment(
        uin=_safe_int(
            _mapping_first(item, COMMENT_UIN_KEYS)
            or _mapping_first(user, COMMENT_USER_UIN_KEYS)
        ),
        nickname=_first_text(
            _mapping_first(user, COMMENT_USER_NICKNAME_KEYS),
            _mapping_first(item, COMMENT_NICKNAME_KEYS),
        ),
