...
class QzoneFeedQueryService(QzoneMethodSet):
    def _recent_posts_params(
        self, ctx: QzoneContext, *, count: int, cursor: str = ""
    ) -> dict[str, Any]:
        params = {
            "uin": ctx.uin,
            "scope": 0,
    ...
    def _friend_feeds_payload_meta(payload: dict[str, Any]) -> dict[str, Any]:
        ...
        return {
            ...
            "data_keys": sorted(str(key) for key in data.keys())[:30]
            if isinstance(data, dict)
            else [],
            "main_keys": sorted(str(key) for key in main.keys())[:30]
