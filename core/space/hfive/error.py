...
class QzoneH5ErrorService(QzoneMethodSet):
    """H5 传输错误识别与格式化。"""
    ...
    def _h5_login_expired_message(message: Any) -> bool:
        ...
        return any(
            token in text
            for token in ("尚未登录", "登录超时", "请先登录", "登录态失效")
        )
    ...
    def _h5_should_retry_http11(status: int, transport: str) -> bool:
        return transport == "HTTP/2" and (
            status in {429, 502, 503, 504} or status >= 500
        )
    ...
    def _h5_transport_error(
        ...
        *,
        ...
        if isinstance(exc, (asyncio.TimeoutError, TimeoutError)):
            return RuntimeError(
                f"QQ 空间 H5 上传请求超时（{self._api_timeout_seconds()}秒）"
            )
