...
class QzoneFeedRecentService(QzoneMethodSet):
    """好友动态列表。"""
    ...
    async def query_recent_posts(
        ...
        *,
        ...
        if not self._ok(payload):
            ...
            raise RuntimeError(
                str(payload.get("message") or "获取 QQ 空间好友动态失败")
            )
        ...
        self._store_query_posts(cache_key, posts, meta=self._last_friend_feeds_meta)
        return posts

    async def _query_recent_post_details(
        self, posts: list[QzonePost]
    ) -> list[QzonePost]:
        detailed = []
        for post in posts:
            try:
