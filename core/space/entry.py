...
def parse_about_me(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    main: dict[str, Any] = raw_main if isinstance(raw_main, dict) else {}
...
def parse_last_year(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "items": posts,
        "has_more": bool(main.get("hasMoreFeeds_0") or main.get("hasMoreFeeds_1"))
        and bool(posts),
        ...
        if isinstance(value, list):
            for image in value:
                if isinstance(image, dict):
                    url = _first_url(
                        image.get("url"), image.get("origin_url"), image.get("pic_url")
                    )
                else:
...
def _fav_link(item: dict[str, Any]) -> str:
    raw_shuoshuo = item.get("shuoshuo_info")
    shuoshuo: dict[str, Any] = raw_shuoshuo if isinstance(raw_shuoshuo, dict) else {}
    owner = _safe_int(
        shuoshuo.get("owner_uin") or item.get("owner_uin") or item.get("uin")
    )
...
def parse_favorites(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    for index, item in enumerate(raw_items if isinstance(raw_items, list) else []):
        if not isinstance(item, dict):
            continue
        raw_shuoshuo = item.get("shuoshuo_info")
        shuoshuo: dict[str, Any] = (
            raw_shuoshuo if isinstance(raw_shuoshuo, dict) else {}
        )
        title = _first_text(
            item.get("title"),
            item.get("abstract"),
        ...
        items.append(
            {
                ...
                "title": title or "收藏内容",
                "summary": _first_text(
                    item.get("abstract"), item.get("desp"), shuoshuo.get("content")
                ),
    ...
    return {
        "items": items,
        "total": _safe_int(data.get("total_num") if isinstance(data, dict) else 0)
        or len(items),
        "has_more": len(items) > 0
        and (
            _safe_int(data.get("total_num")) > len(items)
...
def parse_message_board(payload: dict[str, Any], *, start: int = 0) -> dict[str, Any]:
    data = _payload_data(payload)
    raw_comment_items = data.get("commentList")
    raw_items: list[Any] = (
        raw_comment_items if isinstance(raw_comment_items, list) else []
    )
        ...
        items.append(
            {
                ...
                "time_label": _time_label(
                    item.get("create_time") or item.get("pubtime") or item.get("time")
                ),
