...
class QzoneCommentUtilService(QzoneMethodSet):
    """QQ 空间评论提交与诊断的通用工具。"""
    ...
    def _payload_message(payload: dict) -> str:
        ...
        if not message:
            return ""
        decoded = QzoneCommentUtilService._decode_mojibake(message)
    ...
    def _decode_mojibake(value: str) -> str:
        ...
        for source, target in (
            ...
            try:
                decoded = (
                    text.encode(source, errors="ignore")
                    .decode(target, errors="ignore")
                    .strip()
                )
            except Exception:
                continue
        ...
        def score(item: str) -> int:
            bad = sum(
                item.count(token) for token in ("锟", "�", "璇", "绌", "棿", "宸茶")
            )
    ...
    def _reply_target_unavailable(payload: dict) -> bool:
        if int((payload or {}).get("code") or 0) == -10049:
            return True
        message = str(
            (payload or {}).get("message") or (payload or {}).get("msg") or ""
        )
        return any(
            token in message
            for token in ("该条内容已被删除", "原文已经被删除", "无法查看")
        )
    ...
    def _comment_submit_ok(self, payload: dict) -> bool:
        ...
        if self._write_response_without_json_ok(payload):
            logger.debug(
                "[日常分享] QQ 空间评论接口返回内容不是结构化数据，但接口状态正常，按评论成功处理。"
            )
            return True
        return False
