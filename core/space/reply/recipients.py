...
class QzoneReplyTargetService(QzoneMethodSet):
    """提交前构建并校验 QQ 空间回评目标。"""

    @staticmethod
    def _prefer_h5_reply(
        comment: QzoneComment, *, parent_comment: QzoneComment | None = None
    ) -> bool:
        return bool(
    ...
    def _can_try_addreply_ugc_thread_variant(
        ...
        *,
        ...
        try:
            ...
            return False
        parent_ids = QzoneReplyIdentityService._comment_id_aliases(parent_comment)
        post_uin = int(getattr(post, "uin", 0) or 0)
        parent_uin = int(getattr(parent_comment, "uin", 0) or 0)
        reply_to_uin = int(
            getattr(comment, "reply_to_uin", 0)
            or getattr(comment, "raw_reply_to_uin", 0)
            or 0
        )
        ...
        if parent_uin == self_uin and (not reply_to_tid or reply_to_tid in parent_ids):
            return True
        saw_self_reply_under_parent, matched_target = cls._self_reply_thread_state(
            post, self_uin=self_uin, parent_tid=parent_tid, reply_to_tid=reply_to_tid
        )
        if matched_target:
            return True
    ...
    @classmethod
    def _self_reply_thread_state(
        cls, post: QzonePost, *, self_uin: int, parent_tid: str, reply_to_tid: str
    ) -> tuple[bool, bool]:
        saw_self_reply = False
        for item in getattr(post, "comments", []) or []:
            if int(getattr(item, "uin", 0) or 0) != self_uin:
                continue
            item_parent = str(getattr(item, "parent_tid", "") or "").strip()
            if parent_tid and item_parent != parent_tid:
                continue
            saw_self_reply = True
            if reply_to_tid in QzoneReplyIdentityService._comment_id_aliases(item):
                return True, True
        return saw_self_reply, False
    ...
    def _reply_submit_targets(
        ...
        *,
        ...
        def add(comment_id: str, comment_uin_value: int) -> None:
            ...
            if any(
                (item["comment_id"], item["comment_uin"]) == key for item in targets
            ):
                return
            targets.append(
                {"comment_id": comment_id, "comment_uin": int(comment_uin_value or 0)}
            )
        ...
        *,
        ...
        for target in targets or []:
            ...
            if not comment_id or comment_id in blocked_ids:
                continue
            if parent_comment is not None and cls._is_short_numeric_comment_id(
                comment_id
            ):
                continue
            ...
            if any(
                (item["comment_id"], item["comment_uin"]) == key for item in filtered
            ):
                continue
    ...
    def unsafe_thread_reply_target_reason(
        ...
        *,
        ...
        return cls._unsafe_thread_reply_target_reason(
    ...
    def has_thread_reply_submit_plan(
        ...
        *,
        ...
        if parent_comment is None:
            return False
        try:
            targets = self._reply_submit_targets(
            ...
            return bool(
                self._thread_reply_payload_variants(
    ...
    def _unsafe_thread_reply_target_reason(
        ...
        *,
        ...
        if not comment_tid or "_r_" not in comment_tid or not parent_ids:
            return ""
        if parent_tid and not any(
            comment_tid.startswith(f"{parent_id}_r_") for parent_id in parent_ids
        ):
            return ""
        ...
        if (
            submit_tid
            and submit_tid != raw_tid
            and raw_tid
            and not cls._is_short_numeric_comment_id(raw_tid)
        ):
            return ""
