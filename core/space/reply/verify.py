...
class QzoneReplyVerifyService(QzoneMethodSet):
    async def _verify_thread_reply_submission(
        ...
        *,
        ...
        target_ids = self._reply_verification_target_ids(
    ...
    async def _cleanup_failed_thread_reply(
        ...
        *,
        ...
        try:
            ...
            logger.warning(f"[日常分享] QQ 空间错楼回评自动删除失败: {exc}")
            return {
        ...
        for item in comments:
            ...
            if target_status == "target":
                return {
                    ...
                    "verified_reply_to_tid": str(
                        getattr(item, "reply_to_tid", "") or ""
                    ),
    ...
    def _reply_verification_target_ids(
        ...
        *,
        ...
        target_ids = QzoneReplyIdentityService._comment_id_aliases(comment)
    ...
    @classmethod
    def _reply_verification_existing_self_ids(
        cls, post: QzonePost, self_uin: int
    ) -> set[str]:
        ids: set[str] = set()
        for item in getattr(post, "comments", []) or []:
            if int(getattr(item, "uin", 0) or 0) != int(self_uin or 0):
                continue
            ids.update(QzoneReplyIdentityService._comment_id_aliases(item))
        return ids

    @classmethod
    def _comment_alias_map(
        cls, comments: list[QzoneComment]
    ) -> dict[str, QzoneComment]:
        aliases: dict[str, QzoneComment] = {}
        for item in comments:
            for alias in QzoneReplyIdentityService._comment_id_aliases(item):
    ...
    def _reply_verification_target_status(
        ...
        *,
        ...
        if reply_target is not None:
            reply_ids.update(
                QzoneReplyIdentityService._comment_id_aliases(reply_target)
            )
        ...
        if reply_ids & parent_ids:
            if reply_to_uin and target_uin and reply_to_uin == target_uin:
                return "target"
            return "parent"
        return "other"
    ...
    @classmethod
    def _reply_verification_text_matches(
        cls, item: QzoneComment, expected_texts: set[str]
    ) -> bool:
        if not expected_texts:
            return False
        actual_texts = cls._reply_verification_text_variants(
    ...
    def _reply_verification_is_new_self_comment(
        ...
        *,
        ...
        aliases = QzoneReplyIdentityService._comment_id_aliases(item)
    ...
    @staticmethod
    def _reply_verification_debug_fields(
        ...
        fields = {
            ...
            "verified_reply_to_tid": str(
                verification.get("verified_reply_to_tid") or ""
            ),
            "verified_reply_to_uin": int(
                verification.get("verified_reply_to_uin") or 0
            ),
    ...
    def _attach_reply_failure_debug(
        ...
        *,
        ...
        if verification is not None:
            ...
            for key, value in cls._reply_verification_debug_fields(
                verification
            ).items():
