...
class QzoneReplyPlanService(QzoneMethodSet):
    """准备按顺序尝试的 QQ 空间回评提交方案。"""
    ...
    def _reply_submit_plans(
        ...
        *,
        ...
        if parent_comment is not None:
            if int(getattr(post, "uin", 0) or 0) != int(ctx.uin or 0) and int(
                getattr(parent_comment, "uin", 0) or 0
            ) == int(ctx.uin or 0):
                variants = self._thread_reply_re_feeds_payload_variants(
                    post, comment, parent_comment, reply_targets
                )
                if variants:
                    return [
            ...
            variants = self._thread_reply_payload_variants(
                post, comment, parent_comment, reply_targets
            )
            return [
                self._reply_submit_plan(
        ...
        for target in reply_targets:
            ...
            target_uin = int(
                (target or {}).get("comment_uin") or getattr(comment, "uin", 0) or 0
            )
    ...
    def _thread_reply_payload_variants(
        ...
        if (
            not target_id
            or target_id == post_tid
            or target_id in parent_ids
            or not target_uin
        ):
            return []

        child_seq = submit_tid or raw_tid
        unsafe_reason = QzoneReplyTargetService._unsafe_thread_reply_target_reason(
            comment, parent_comment=parent_comment
        )
        if (
            unsafe_reason
            and not QzoneReplyTargetService._can_try_addreply_ugc_thread_variant(
                post,
                comment,
    ...
    def _reply_submit_plan(
        ...
        *,
        ...
        comment_uin = int(
            (variant or {}).get("comment_uin") or getattr(comment, "uin", 0) or 0
        )
        payload_comment_id = str(
            (variant or {}).get("payload_comment_id") or comment_id
        ).strip()
        payload_comment_uin = int(
            (variant or {}).get("payload_comment_uin") or comment_uin or 0
        )
        payload_t2_tid = str(
            (variant or {}).get("payload_t2_tid") or payload_comment_id
        ).strip()
        payload_t2_uin = int((variant or {}).get("payload_t2_uin") or comment_uin or 0)
        reply_uin = int(
            (variant or {}).get("reply_uin") or payload_t2_uin or comment_uin or 0
        )
        ...
        if transport == "sns":
            ...
            headers = self._comment_sns_headers(
                ctx, referer=f"{self.BASE_URL}/{post.uin}/mood/{post.tid}"
            )
            ...
            headers = self._comment_h5_headers(
                ctx, referer=str(data.get("qzreferrer") or "")
            )
            ...
            url = self.H5_COMMENT_URL
            headers = self._comment_h5_headers(
                ctx, referer=f"{self.H5_ORIGIN}/{post.uin}/mood/{post.tid}"
            )
        else:
            data = {
                "hostUin": post.uin,
            ...
            if payload_t2_tid and comment_uin and bool((variant or {}).get("thread")):
                data.update(
                    t1_uin=post.uin,
                    t1_tid=post.tid,
                    t2_tid=payload_t2_tid,
                    t2_uin=payload_t2_uin,
                )
