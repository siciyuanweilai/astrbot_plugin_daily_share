...
from .common import _PAGE_CLOCK_TIME_RE, _PAGE_RANDOM_PERIOD_RE, _PAGE_SHARE_TYPE_OPTIONS
...
class DashboardConfigValidationMixin:
    ...
    def _page_type_list_value(self, value: Any, label: str) -> list:
        ...
        raw_items = self._page_list_value(value, max_items=len(allowed), item_max_len=32, split_commas=True)
    ...
    def _page_sequence_value(self, value: Any, default: list, label: str) -> list:
        allowed = _PAGE_SHARE_TYPE_OPTIONS - {"自动"}
        raw_items = self._page_ordered_list_value(
            value if value not in (None, "") else default,
            max_items=24,
            item_max_len=32,
            split_commas=True,
