...
class DashboardBaseMixin:
    ...
    def _register_page_web_apis(self) -> None:
        routes = (
            ("page/media/delete", self.page_media_delete, ["POST"], "删除每日分享媒体记录"),
            ...
            ("page/events", self.page_events, ["GET"], "每日分享仪表盘事件"),
            ("page/qzone/feed", self.page_qzone_feed, ["GET"], "QQ 空间动态"),
            ("page/qzone/detail", self.page_qzone_detail, ["GET"], "QQ 空间说说详情"),
            ("page/qzone/relation", self.page_qzone_relation, ["GET"], "QQ 空间在意好友"),
            ("page/qzone/entry", self.page_qzone_entry, ["GET"], "QQ 空间扩展入口"),
            ("page/qzone/upload-media", self.page_qzone_upload_media, ["POST"], "上传 QQ 空间说说媒体"),
            ("page/qzone/publish", self.page_qzone_publish, ["POST"], "发布 QQ 空间说说"),
            ("page/qzone/like", self.page_qzone_like, ["POST"], "点赞 QQ 空间说说"),
            ("page/qzone/comment", self.page_qzone_comment, ["POST"], "评论 QQ 空间说说"),
            ("page/qzone/delete", self.page_qzone_delete, ["POST"], "删除 QQ 空间说说"),
        ...
        for endpoint, handler, methods, desc in routes:
            self.context.register_web_api(
                f"/astrbot_plugin_daily_share/{endpoint}",
                handler,
                methods,
                desc,
            )
