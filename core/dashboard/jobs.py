...
class DashboardJobsMixin:
    ...
    def _page_job_display_name(
        ...
        def clean_smart_name(value: str) -> str:
            return re.sub(r"(智能定时)\s+\d{1,2}:\d{2}(?=\s*·|$)", r"\1", str(value or "")).strip()
        ...
        for pattern, kind, offset, is_target in patterns:
            ...
            if is_target:
                ...
                try:
                    ...
                    pass
            if kind == "auto_share_fixed":
                return f"全局固定时间 {value}"
            if kind == "briefing_fixed":
                return f"早报固定时间 {value}"
            if kind == "qzone_fixed":
                return f"QQ 空间固定时间 {value}"
            if kind == "random":
                return f"随机分享 {value}"
            if kind == "qzone_random":
                return f"QQ 空间 · 随机分享 {value}"
            if kind == "smart":
                return clean_smart_name(job_name) or f"全局智能定时 {value}"
            if kind == "briefing_smart":
                return clean_smart_name(job_name) or f"早报智能定时 {value}"
            if kind == "qzone_smart":
                return clean_smart_name(job_name) or f"QQ 空间 · 智能定时 {value}"
            if kind == "custom":
                return f"{value} · 独立分享"
