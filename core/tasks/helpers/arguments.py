...
class TaskHelperArgsService(TaskHelperContextService):
    ...
    def event_history_target(self, event: AstrMessageEvent) -> str:
        if not event:
            return ""
        try:
    ...
    def map_share_type_arg(self, share_type_text: str):
    ...
    def map_news_source_arg(self, source: str):
        if not source:
            return None
