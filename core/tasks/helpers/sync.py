...
class TaskHelperSyncService(TaskServiceBase):
    """QQ 空间发布结果同步到触发会话。"""

    async def sync_qzone_result_to_event(
        self,
        ...
        if target_uid and self.ctx_service.is_weixin_platform(target_uid):
            ok = await self.services.delivery.send(
                target_uid,
                text,
                img_path,
                video_url=video_url,
            ...
            if not ok:
                logger.error("[日常分享] 同步发送内容到会话失败")
            return

        text_chain = MessageChain().message(text)
        await self.send_event(event, text_chain)
        ...
        if img_path:
            ...
            await self.send_event(event, img_chain)
        if video_url:
            await asyncio.sleep(1.0)
            ...
            await self.send_event(event, video_chain)
