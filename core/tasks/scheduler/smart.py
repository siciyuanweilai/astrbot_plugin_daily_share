...
class TaskSchedulerSmartService(SchedulerComponent):
    ...
    def _smart_normalize_jobs(
        ...
        *,
        ...
        for item in raw_jobs:
            normalized = self._smart_normalize_job(
                item,
                now=now,
                quiet_hours=quiet_hours,
            ...
            if not normalized:
                continue
            run_at, job = normalized
            minute_key = run_at.strftime("%Y-%m-%d %H:%M")
            too_close = any(
                abs((run_at - accepted).total_seconds()) < min_gap_minutes * 60
                for accepted in accepted_times
            )
            if minute_key in seen_minutes or too_close:
                continue
            result.append(job)
    ...
    def _smart_normalize_job(
        ...
        *,
        ...
        if not isinstance(item, dict):
            return None
        raw_time = item.get("run_at") or item.get("time") or item.get("datetime")
        run_at = self._smart_today_time(now, raw_time)
        if not run_at or run_at.date() != now.date():
            return None
        run_at = self._smart_naturalize_seconds(run_at)
        if run_at <= now + timedelta(seconds=30) or self._smart_quiet_contains(
            run_at, quiet_hours
        ):
            return None
        share_type = (
            str(item.get("share_type") or item.get("type") or "自动").strip() or "自动"
        )
        if not allow_share_type or (
            share_type != "自动" and not self._smart_share_type(share_type)
        ):
            share_type = "自动"
        return run_at, {
    ...
    async def _smart_recent_history_summary(self, task_kind: str) -> str:
        try:
            if task_kind == "qzone":
                records = await self.db.get_recent_history_by_target(
                    QZONE_TARGET_ID, limit=6
                )
            else:
        ...
        for item in records or []:
            ...
            target = (
                "QQ空间"
                if item.get("target_id") == QZONE_TARGET_ID
                else str(item.get("target_id") or "未知目标")
            )
    ...
    async def _generate_llm_smart_jobs(
        ...
        *,
        ...
        share_types = (
            "自动、问候、新闻、心情、知识、推荐" if allow_share_type else "自动"
        )
        ...
        jobs = await asyncio.to_thread(self._smart_json_loads, result)
    ...
    def _clear_smart_scheduler_jobs(self, job_prefix: str) -> None:
        for job in list(self.scheduler.get_jobs()):
            job_id = job.id
            if str(job_id).startswith(job_prefix):
                self.scheduler.remove_job(job_id)

    def _load_cached_smart_jobs(
        self,
        smart_schedule: Any,
        *,
        date_str: str,
    ...
    async def _generate_and_store_smart_jobs(
        self,
        *,
        state_key: str,
        task_kind: str,
        ...
        try:
            ...
            logger.warning(
                f"[日常分享] {label}智能定时生成失败，今日不安排智能定时任务: {error}"
            )

        if not jobs:
            error = error or "智能定时未返回可用计划"
            logger.warning(
                f"[日常分享] {label}智能定时未生成可用计划，今日不安排智能定时任务: {error}"
            )
    ...
    def _register_smart_scheduler_jobs(
        self,
        *,
        jobs: list[dict],
        now: datetime,
        ...
        for index, item in enumerate(jobs):
            ...
            logger.debug(
                f"[日常分享] {label}智能定时已安排: {run_time.strftime('%H:%M')} "
    ...
    async def _schedule_daily_llm_smart_jobs(
        ...
        *,
        ...
        self._clear_smart_scheduler_jobs(job_prefix)
        now = datetime.now()
        ...
        smart_schedule = (
            state.get("smart_schedule", {}) if isinstance(state, dict) else {}
        )
        jobs = self._load_cached_smart_jobs(
            smart_schedule,
            date_str=date_str,
            signature=signature,
            now=now,
        ...
        if jobs is None:
            jobs = await self._generate_and_store_smart_jobs(
                state_key=state_key,
                task_kind=task_kind,
                label=label,
                now=now,
        ...
        if jobs is None or not self.schedule.is_current_build(generation):
            return

        self._register_smart_scheduler_jobs(
            jobs=jobs,
            now=now,
            task_kind=task_kind,
            label=label,
    ...
    async def _execute_llm_smart_schedule_item(
        self, task_kind: str, item: dict
    ) -> None:
        ...
        if task_kind == "briefing":
            ...
            async def run_briefing():
                logger.info("[日常分享] 开始智能定时早报任务...")
                await self.services.briefing.execute_briefing_share(
        ...
        if task_kind == "qzone":
            ...
            async def run_qzone():
                logger.info("[日常分享] 开始智能定时 QQ 空间任务...")
                await self.services.qzone_share.execute_qzone_share(
                    force_type=share_type, source_type=source_type
                )
        ...
        async def before_share():
            now = datetime.now()
            last_share_time = self.state.last_share_time
            if last_share_time:
                if (now - last_share_time).total_seconds() < 60:
                    logger.debug("[日常分享] 检测到近期已分享，跳过本次智能定时触发。")
                    return False
            self.state.last_share_time = now
            return True

        async def run_share():
            logger.info("[日常分享] 开始智能定时分享任务...")
            await self.services.share.execute_share(
                force_type=share_type, source_type=source_type
            )
    ...
    async def _schedule_daily_briefing_smart_jobs(
    ...
    async def _schedule_daily_qzone_smart_jobs(self, *, generation: int | None = None):
        await self._schedule_daily_llm_smart_jobs(
