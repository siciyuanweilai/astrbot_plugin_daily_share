...
class TaskSchedulerTriggerService(SchedulerComponent):
    """定时任务触发入口。"""
    ...
    async def _task_wrapper(self):
        ...
        try:
            ...
            logger.warning(f"[日常分享] 数据库清理失败: {e}")
    ...
    async def _execute_delayed_task(self):
        ...
        async def before_share():
            now = datetime.now()
            if self.state.last_share_time:
                if (now - self.state.last_share_time).total_seconds() < 60:
                    logger.debug("[日常分享] 检测到近期已分享，跳过本次触发。")
                    return False
            self.state.last_share_time = now
            return True

        async def run_share():
            logger.info("[日常分享] 开始分享任务...")
    ...
    async def _task_wrapper_qzone(self):
        ...
        random_delay_min = self._scheduled_random_delay_minutes(
            ...
            delay_conf=self.qzone_conf,
            delay_key="qzone_cron_random_delay",
        )
        await self.schedule.delay._schedule_or_execute_delayed(
    ...
    async def _task_wrapper_qzone_auto_interaction(self):
        ...
        async def execute_delayed_qzone_auto_interaction():
            async def run_qzone_auto_interaction():
                logger.info("[日常分享] 开始 QQ 空间自动互动任务...")
                await self.services.qzone_interaction.execute_qzone_auto_interaction()
        ...
        await self.schedule.delay._schedule_or_execute_delayed(
            state_key=QZONE_AUTO_INTERACTION_STATE_KEY,
            state_updater=self.db.update_qzone_state,
            delay_minutes=self.schedule.delay._read_delay_minutes(
                self.qzone_conf, "qzone_cron_random_delay"
            ),
