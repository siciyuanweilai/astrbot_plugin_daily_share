...
def _fit_auto_comment_text_bytes(text: str, max_bytes: int) -> str:
    ...
    if candidate and candidate[-1:] not in "。！？!?":
        terminal = "。" if any("\u4e00" <= char <= "\u9fff" for char in candidate) else "."
        with_period = f"{candidate}{terminal}"
        if len(with_period.encode("utf-8")) <= max_bytes:
            return with_period
        base = _trim_text_utf8_bytes(candidate, max_bytes - len(terminal.encode("utf-8"))).rstrip("，,、；;：: ")
        if len(base) >= 4:
...
def _clean_auto_comment_text(value: Any, *, max_bytes: int = 0) -> str:
    ...
    text = text.strip("`\"'“”‘’")
    text = re.sub(r"^(?:评论|回复|回评|内容|评论正文|回复正文|回评正文)\s*[:：]\s*", "", text).strip()
    text = text.strip("`\"'“”‘’")
    text = _strip_qzone_mention_tags(text)
...
def _is_skip_auto_comment(value: str) -> bool:
    ...
    return text in {"跳过", "不评论", "略过", "skip", "pass"}
