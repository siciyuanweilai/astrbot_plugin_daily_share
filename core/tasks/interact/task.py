...
def _qzone_service(owner):
    return owner.plugin.qzone_service


def _qzone_query_fetch_count(limit: int, multiplier: int) -> int:
    return min(
        QZONE_QUERY_MAX_POSTS, max(QZONE_QUERY_MIN_POSTS, int(limit or 0) * multiplier)
    )


def _qzone_auto_config(owner) -> QzoneAutoInteractionConfig:
    return owner._qzone_auto_config()
...
def _qzone_should_log_summary(result: dict, *success_keys: str) -> bool:
    ...
    return bool(
        ...
        or int(result.get("generation_failed", 0) or 0)
    )
...
async def _qzone_finish_task(
    ...
    *,
    ...
    await owner._qzone_auto_save_state(
        state_key, state, processed, result, run_at=run_at
    )
    if not (emit_summary and _qzone_should_log_summary(result, *success_keys)):
        return
    logger.info(
        f"[日常分享] QQ 空间{task_label}完成: "
...
async def _qzone_prepare_task(
    ...
    *,
    ...
    result["enabled"] = True
    now = int(time.time())
    state = await owner.db.get_qzone_state(state_key, {})
