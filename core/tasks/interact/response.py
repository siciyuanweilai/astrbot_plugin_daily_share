...
def _qzone_reply_base_fields(
    comment, reply: str, *, parent_comment_id: str = ""
) -> dict:
    fields = {
        "content": reply,
        "commenter": str(
...
def _qzone_reply_exception_fields(
    comment, reply: str, exc: Exception, *, parent_comment_id: str = ""
) -> dict:
    fields = _qzone_reply_base_fields(
        comment, reply, parent_comment_id=parent_comment_id
    )
    fields["attempted_targets"] = _qzone_dict_list(
        getattr(exc, "attempted_targets", None)
    )
...
def _qzone_submitted_reply_fields(
    comment, reply: str, submit_result: Any, *, parent_comment_id: str = ""
) -> dict:
    fields = _qzone_reply_base_fields(
        comment, reply, parent_comment_id=parent_comment_id
    )
