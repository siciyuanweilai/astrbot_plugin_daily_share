    ...
    *,
    ...
    for candidate in candidates:
        ...
        if pending_reply:
            ...
            try:
                reply = (
                    await owner._generate_qzone_auto_reply_thread(
                        post, parent_comment, comment, state=state
                    )
                    if is_thread_reply
                    else await owner._generate_qzone_auto_reply(
                        post, comment, state=state
                    )
                )
            except Exception as exc:
                result["failed"] += 1
                result["generation_failed"] += 1
                logger.warning(f"[日常分享] QQ 空间自动回评生成失败: {exc}")
                continue
        ...
        try:
            await owner._qzone_send_auto_reply_result(
                ...
                item_key=item_key,
                reply=reply,
                processed_action=QZONE_ACTION_THREAD_REPLIED
                if is_thread_reply
                else QZONE_ACTION_REPLIED,
                dashboard_action="auto_reply",
                log_label="已自动回评 QQ 空间评论",
                parent_comment_id=(
                    str(getattr(parent_comment, "tid", "") or "")
                    if is_thread_reply
                    else ""
                ),
            ...
            logger.debug(f"[日常分享] QQ 空间自动回评稍后重试: {exc}")
            break
        except Exception as exc:
            result["failed"] += 1
            logger.warning(f"[日常分享] QQ 空间自动回评失败: {exc}")
...
async def execute_qzone_auto_reply_task(owner, *, emit_summary: bool = True) -> dict:
    ...
    ready, state, processed, now = await _qzone_prepare_task(
        ...
        disabled_log="[日常分享] QQ 空间自动回评未开启，跳过。",
    )
    if not ready:
        return result
    try:
