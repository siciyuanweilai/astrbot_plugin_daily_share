...
async def execute_qzone_auto_like_task(
    owner, *, emit_summary: bool = True, target_id: str = ""
) -> dict:
    ...
    ready, state, processed, now = await _qzone_prepare_task(
        ...
        disabled_log="[日常分享] QQ 空间自动点赞未开启，跳过。",
    )
    if not ready:
        return result
    ...
    for post in posts:
        ...
        if not owner._qzone_auto_like_candidate(
            post, self_uin=ctx.uin, processed=processed
        ):
            result["skipped"] += 1
            continue
        ...
        try:
            ...
            if _qzone_is_retry_later_error(exc):
                ...
                logger.debug(f"[日常分享] QQ 空间自动点赞稍后重试: {exc}")
                break
            result["failed"] += 1
            logger.warning(f"[日常分享] QQ 空间自动点赞失败: {exc}")
