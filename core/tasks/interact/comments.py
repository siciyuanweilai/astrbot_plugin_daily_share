...
def _comment_mention_uins(comment) -> set[int]:
    ...
    return {
        uin
        for uin in (
            _safe_int(match) for match in re.findall(r"@\{[^}]*\buin:(\d+)", text, re.I)
        )
        if uin
    }
...
class QzoneCommentIndex:
    ...
    def has_timed_self_reply(self, parent_tid: str) -> bool:
        return any(
            ...
            for item in self.self_replies_by_parent.get(
                str(parent_tid or "").strip(), []
            )
        )
    ...
    def self_reply_bounds(self, parent_tid: str, comment) -> tuple[bool, bool]:
        ...
        comment_index = self._comment_position(comment)
        ...
        for item in self.self_replies_by_parent.get(parent_tid, []):
            ...
            if comment_time and item_time:
                if item_time < comment_time:
                    ...
                    has_after = True
                continue
            item_index = self._comment_position(item)
            if comment_index < 0:
                has_after = True
            elif 0 <= item_index < comment_index:
                has_before = True
        ...
        for item in self.by_parent_tid_nonself.get(parent_tid, []):
            ...
            if comment_time and item_time:
                if item_time > comment_time:
                    return True
                continue
            if comment_index >= 0 and self._comment_position(item) > comment_index:
                return True
        return False

    def _comment_position(self, comment) -> int:
        tid = _comment_tid(comment)
        for index, item in enumerate(self.comments):
            if item is comment or (tid and _comment_tid(item) == tid):
                return index
        return -1


def _post_has_self_comment(
    post, self_uin: int, index: QzoneCommentIndex | None = None
) -> bool:
    self_uin = _safe_int(self_uin)
    if not self_uin:
        return False
...
def _comment_has_self_reply(
    post, comment, self_uin: int, index: QzoneCommentIndex | None = None
) -> bool:
    self_uin = _safe_int(self_uin)
...
def _comment_thread_has_self_reply_to(
    ...
    if not self_uin or not target_tid:
        return False
    return (index or QzoneCommentIndex.build(post, self_uin)).has_self_reply_to(
        target_tid
    )


def _comment_replies_to_self(
    comment, self_uin: int, index: QzoneCommentIndex | None = None
) -> bool:
    self_uin = _safe_int(self_uin)
    if not self_uin:
        return False
...
def _comment_thread_has_timed_self_reply(
    ...
    if not self_uin or not parent_tid:
        return False
    return (index or QzoneCommentIndex.build(post, self_uin)).has_timed_self_reply(
        parent_tid
    )
...
def _comment_thread_self_reply_bounds(
    ...
    if not self_uin or not parent_tid:
        return False, False
    return (index or QzoneCommentIndex.build(post, self_uin)).self_reply_bounds(
        parent_tid, comment
    )
...
def _comment_thread_has_later_nonself_reply(
    ...
    if not parent_tid:
        return False
    return (index or QzoneCommentIndex.build(post, self_uin)).has_later_nonself_reply(
        parent_tid, comment, self_uin
    )
