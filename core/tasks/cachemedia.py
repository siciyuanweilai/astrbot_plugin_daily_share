...
class TaskDeliveryAssetsMixin:
    ...
    def _infer_news_image_source_key(self, url: str = None, source_name: str = None) -> str:
        ...
        try:
            ...
            logger.debug(f"[每日分享] 识别新闻源图片来源失败: {format_exception(exc)}")
    ...
    def _news_image_extension_from_url(self, url: str = None) -> str:
        try:
            ...
            logger.debug(f"[每日分享] 识别新闻源图片扩展名失败: {format_exception(exc)}")
        return ".png"
    ...
    def _remove_file_quietly(path: str, *, label: str) -> None:
        try:
            ...
            logger.debug(f"[每日分享] {label}失败: {path}, {format_exception(exc)}")
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, timeout=timeout_sec) as resp:
                    if resp.status == 200:
                        ...
                        return temp_path
                    logger.warning(f"[每日分享] 图片下载失败，接口状态码: {resp.status}")
        except Exception as e:
            if temp_path:
                self._remove_file_quietly(temp_path, label="清理未完成下载图片")
            logger.warning(f"[每日分享] 图片下载异常: {format_exception(e)}")
        return None
    ...
    async def _prepare_qzone_image(self, image_ref):
        ...
        if os.path.exists(image_ref):
            try:
                ...
                logger.warning(f"[每日分享] 读取 QQ 空间本地配图失败: {format_exception(e)}")
                return None
    ...
    async def cleanup_news_source_images(self):
        ...
        try:
            ...
            logger.warning(f"[每日分享] 新闻源图片清理失败: {format_exception(e)}")
    ...
    async def cleanup_weixin_temp_images(self):
        ...
        try:
            ...
            logger.warning(f"[每日分享] 个人微信压缩图清理失败: {format_exception(e)}")
