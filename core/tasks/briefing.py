...
class TaskBriefingMixin:
    ...
    async def _send_command_briefing_image(
        ...
        *,
        ...
        target_id = QZONE_TARGET_ID if to_qzone else self._event_history_target(event)
        ...
        if to_qzone:
            try:
                ...
                await self.plugin._safe_publish_qzone(text=qzone_text, images=[url])
    ...
    async def execute_briefing_share(self, specific_target: str = None, source_type: str = SOURCE_SCHEDULED):
        ...
        if specific_target is None and self.extra_shares_conf.get("sync_briefing_to_qzone", False):
            ...
            for name, original_url, local_path in images_to_send:
                ...
                try:
                    logger.info(f"[每日分享] 正在登录 QQ 空间，准备分享早报 {name}...")
                    self._update_share_progress(
                        progress_id,
                        "send",
                        message="正在登录 QQ 空间",
                    )
                    await self.plugin._safe_publish_qzone(text=title, images=[original_url])
                    await self.db.add_sent_history(
                        QZONE_TARGET_ID,
                        HISTORY_SHARE_BRIEFING,
                        f"{title}(定时自动)",
