...
class TaskDeliveryWeixinMixin:
    ...
    def _apply_weixin_timeout(self, platform_inst):
        """按插件配置调高个人微信接口和资源上传超时，避免大图上传被 15 秒默认值截断。"""
        ...
        try:
            ...
            logger.debug(f"[每日分享] 设置个人微信平台超时失败: {e}")
    ...
    def _compress_image_for_weixin_sync(
        ...
        try:
            ...
            logger.debug(f"[每日分享] 图片处理库不可用，跳过微信图片压缩: {e}")
            return img_path
        ...
        try:
            with PILImage.open(img_path) as im:
                ...
                if out_size < raw_size:
                    logger.info(
                        f"[每日分享] 已为个人微信平台优化图片: {raw_size / 1024 / 1024:.2f} 兆字节 -> "
                        f"{out_size / 1024 / 1024:.2f} 兆字节，分辨率 {width}×{height} -> {im.size[0]}×{im.size[1]}"
                    )
