...
class DatabaseDashboardMixin:
    ...
    def _dynamic_filter_clause(
        ...
        cutoff = self._today_cutoff() if today_only else self._days_cutoff(days)
        if cutoff:
            clauses.append("created_at >= ?")
            params.append(cutoff)

        raw_type = str(share_type or "").strip()
        normalized_type = raw_type.lower()
        if raw_type and normalized_type != "all":
            clauses.append("share_type = ?")
    ...
    def _sync_get_recent_dynamics(
        ...
        conn = self._get_conn()
        cursor = conn.cursor()
        filter_clause, params = self._dynamic_filter_clause(days, media_kind, share_type, today_only)
        params.append(limit)
        cursor.execute(f'''
            SELECT {_HISTORY_SELECT_COLUMNS}
            FROM sent_history
    ...
    def _sync_get_target_stats(self, days: int = 30, briefing: Optional[bool] = None) -> List[Dict]:
        ...
        cursor.execute(f'''
            SELECT target_id, share_type, COUNT(*) AS type_count
            FROM sent_history
            WHERE success = 1
              {type_scope_clause}
            GROUP BY target_id, share_type
        ''')
        type_rows = cursor.fetchall()

        type_counts = {}
        for target_id, share_type, count in type_rows:
            type_counts.setdefault(target_id, {})[share_type] = count
