...
async def _get_message_id(event: AstrMessageEvent) -> int | None:
    ...
    try:
        ...
        logger.debug(f"[日常分享][表情标记] 获取消息标识失败: {exc}")
    return None
...
async def set_emoji(
    ...
    try:
        ...
        logger.debug(
            f"[日常分享][表情标记] 已标记消息，消息标识={message_id}, 表情标识={emoji_id}"
        )
        return True
    except Exception as exc:
        logger.debug(f"[日常分享][表情标记] 标记失败: {exc}")
        return False
