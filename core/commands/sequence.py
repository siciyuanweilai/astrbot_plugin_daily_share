...
from ..constants import TYPE_CN_MAP, normalize_share_type_sequence, normalize_share_type_token, period_label
...
class CommandSequenceMixin:
    async def cmd_reset_seq(self, event: AstrMessageEvent):
        ...
        if is_qzone:
            ...
            await self.db.update_state_dict(QZONE_STATE_KEY, qzone_updates)
    ...
    async def cmd_view_seq(self, event: AstrMessageEvent):
        ...
        if not is_qzone:
            ...
            if conf is not None:
                target_specific_type = conf.get("seq", "自动") if isinstance(conf, dict) else conf
        else:
        ...
        if target_specific_type:
            custom_seq = normalize_share_type_sequence(target_specific_type, allow_auto=True)
    ...
    async def cmd_set_seq(self, event, parts):
        ...
        if len(parts) > 2 and parts[2].isdigit():
            ...
            if 0 <= target_idx < len(seq):
                ...
                yield event.plain_result(f"序号无效，当前时段[{period_label(period)}] 范围: 0 ~ {len(seq)-1}")
        else:
