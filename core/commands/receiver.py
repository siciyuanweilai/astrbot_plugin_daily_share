    ...
    async def cmd_add_current(self, event: AstrMessageEvent, parts: list):
        ...
        if len(parts) > 2:
            ...
            if not self.plugin.task_manager._looks_like_share_sequence(seq_candidate):
                yield event.plain_result("类型序列格式不正确。示例：/分享 添加当前 心情,新闻\n添加早报示例：/分享 添加当前 早报")
                return
            seq = self.plugin.task_manager._normalize_share_sequence(seq_candidate)
        ...
        # 配置保存纯会话标识：QQ 为 QQ 号，个人微信为开放标识，实际发送时再按平台拼完整会话标识。
