...
DEFAULT_SKILLS_REF = "v0.1.0-b043748"
MANAGED_UV_PATH = (
    Path.home() / ".local" / "share" / "daily-share-xhs" / "uv" / "bin" / "uv"
)
...
def _install_system_packages(*, dry_run: bool) -> list[str]:
    ...
    if not manager:
        supported = "、".join(
            item.replace("apt-get", "apt") for item in PACKAGE_MANAGERS
        )
...
def _docker_info(container: str) -> dict[str, Any]:
    ...
    for network in networks.values() if isinstance(networks, dict) else []:
        ...
        if address and address not in result["addresses"]:
...
def _inspect(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not skills:
        issues.append(
            "未找到包含 scripts/cli.py 和 extension/manifest.json 的小红书 CLI 目录"
        )
    if not uv:
    ...
    allowed_clients = [
        ...
        for item in dict.fromkeys(
            [
                *([host] if host in {"127.0.0.1", "::1"} else []),
                *docker.get("addresses", []),
                *getattr(args, "allow_client", []),
            ]
        )
        if item
    ]
    if host not in {"127.0.0.1", "::1"} and not allowed_clients:
        issues.append("非回环监听必须至少配置一个允许访问的客户端 IP")
...
def _bootstrap(args: argparse.Namespace) -> int:
    ...
    system_actions = (
        _install_system_packages(dry_run=args.dry_run)
...
def _unit_path(value: str | Path) -> str:
    ...
    if any(char in text for char in ("\0", "\n", "\r")):
        raise RuntimeError("systemd 路径包含无效字符")
    return (
        text.replace("%", "%%")
        .replace("\\", "\\x5c")
        .replace(" ", "\\x20")
...
def _apparmor_profile(browser: str | Path) -> str:
    executable = str(browser).replace("\\", "\\\\").replace(" ", "\\x20")
    return (
        "abi <abi/4.0>,\n"
        "include <tunables/global>\n\n"
        f"profile daily-share-xhs-chromium {executable} flags=(unconfined) {{\n"
        "    userns,\n"
        "}\n"
    )


def _install_apparmor_profile(browser: str | Path, *, dry_run: bool = False) -> str:
    ...
    try:
        ...
        restricted = False
    if not restricted:
        return "系统未限制非特权 userns，无需安装 Chromium AppArmor 配置"
    parser = shutil.which("apparmor_parser")
    if not parser:
        return "未找到 apparmor_parser，跳过 Chromium AppArmor userns 配置"
    profile_path = Path("/etc/apparmor.d/daily-share-xhs-chromium")
    if dry_run:
        return f"准备安装 Chromium AppArmor userns 配置: {profile_path}"
    with tempfile.NamedTemporaryFile("w", encoding="utf-8", delete=False) as handle:
        handle.write(_apparmor_profile(browser))
        source = Path(handle.name)
    try:
        _command(
            ["sudo", "install", "-m", "0644", str(source), str(profile_path)],
            check=True,
        )
        _command(["sudo", parser, "-r", str(profile_path)], check=True)
    finally:
        source.unlink(missing_ok=True)
    return f"已安装 Chromium AppArmor userns 配置: {profile_path}"
...
def _units(info: dict[str, Any]) -> dict[str, str]:
    ...
    for address in info["allowed_clients"]:
        bridge_args.extend(["--allow-client", address])
    browser_args = [
...
[Service]
Type=simple
WorkingDirectory={_unit_path(info["plugin_dir"])}
...
[Service]
Type=simple
WorkingDirectory={_unit_path(info["skills_dir"])}
...
def _write_unit(path: Path, content: str, *, force: bool) -> str:
    if path.exists():
        ...
        if not force:
            raise RuntimeError(
                f"服务文件已存在且内容不同，使用 --force 才能备份并替换: {path}"
            )
...
def _install(args: argparse.Namespace) -> int:
    ...
    if args.dry_run:
        print(
            json.dumps(
                {"inspection": info, "units": units}, ensure_ascii=False, indent=2
            )
        )
        return 0
    ...
    profile.chmod(0o700)
    apparmor_action = _install_apparmor_profile(info["browser"])
...
def _cli_call(
    ...
    completed = _command(
        [str(uv), "run", "python", "scripts/cli.py", command], cwd=skills
    )
    if completed.stdout:
...
def _http_json(
    url: str, *, method: str = "GET", timeout: int = 15, token: str = ""
) -> dict[str, Any]:
...
def _container_http_json(
    ...
    *,
    ...
    code = (
        ...
        "t=sys.argv[2];"
        "h.update({'Authorization':'Bearer '+t} if t else {});"
        "q=urllib.request.Request(sys.argv[1],data=(b'{}' if sys.argv[3]=='POST' else None),headers=h,method=sys.argv[3]);"
        'exec("try:\\n r=urllib.request.urlopen(q,timeout=int(sys.argv[4]));print(r.status);print(r.read().decode())\\nexcept urllib.error.HTTPError as e:\\n print(e.code);print(e.read().decode());raise SystemExit(1)")'
    )
    completed = _command(
        [
            "docker",
            ...
            token,
            method,
            str(max(5, min(int(timeout or 120), 600))),
        ]
    )
...
def _verify(args: argparse.Namespace) -> int:
    info = _inspect(args)
    base = (
        args.url.strip().rstrip("/")
        if args.url
        else info["recommended_config"]["server_url"]
    )
    if not base:
        print(
            json.dumps(
                {"ok": False, "error": "无法确定发布服务地址"},
                ensure_ascii=False,
                indent=2,
            )
        )
    ...
    container = (
        _container_health(args.container, f"{base}/health")
        if args.container
        else {
            "ok": True,
            "skipped": True,
    ...
    login = (
        ...
        if args.container and container.get("ok") and not container.get("skipped")
        else _http_json(
            f"{base}/check-login",
            method="POST",
            timeout=args.timeout,
    ...
    logged_in = (
        bool(login_data.get("logged_in")) if isinstance(login_data, dict) else False
    )
...
def _parser() -> argparse.ArgumentParser:
    ...
    common.add_argument(
        ...
        action="append",
        default=[],
        help="额外允许访问桥接服务的精确客户端 IP，可重复指定",
    )
    bootstrap_common = argparse.ArgumentParser(add_help=False)
    bootstrap_common.add_argument(
        "--target-dir", default=str(Path.home() / "xiaohongshu-skills")
    )
    bootstrap_common.add_argument("--repo", default=DEFAULT_SKILLS_REPOSITORY)
    bootstrap_common.add_argument("--ref", default=DEFAULT_SKILLS_REF)
    bootstrap_common.add_argument("--install-system-deps", action="store_true")
    sub = parser.add_subparsers(dest="command", required=True)
    bootstrap = sub.add_parser(
        "bootstrap",
        parents=[bootstrap_common],
        help="下载小红书 CLI 并安装其 Python 依赖",
    )
    bootstrap.add_argument("--uv", default="")
    bootstrap.add_argument("--dry-run", action="store_true")
    sub.add_parser("inspect", parents=[common], help="只读检查环境并给出推荐配置")
    install = sub.add_parser(
        "install", parents=[common], help="安装并启动用户级 systemd 服务"
    )
    install.add_argument(
        "--unit-dir", default=str(Path.home() / ".config" / "systemd" / "user")
    )
    install.add_argument("--force", action="store_true")
    install.add_argument("--dry-run", action="store_true")
    setup = sub.add_parser(
        "setup",
        parents=[common, bootstrap_common],
        help="自动安装依赖并启动小红书后台服务",
    )
    setup.add_argument(
        "--unit-dir", default=str(Path.home() / ".config" / "systemd" / "user")
    )
    ...
    verify = sub.add_parser(
        "verify", parents=[common], help="验证服务、容器连通和登录状态"
    )
    verify.add_argument("--url", default="")
    verify.add_argument("--timeout", type=int, default=120)
    verify.add_argument("--token", default=os.environ.get("XHS_BRIDGE_TOKEN", ""))
    return parser
