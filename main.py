...
class DailySharePlugin(PluginRuntimeMixin, PluginLlmMixin, PluginSupportMixin, DashboardBackendMixin, Star):
    ...
    async def initialize(self) -> None:
        await PluginRuntimeMixin.initialize(self)

    async def terminate(self) -> None:
        # AstrBot 4.25 只有在插件类中显式声明终止方法时才会调用它。
        await PluginRuntimeMixin.terminate(self)
    ...
    async def daily_share_tool(
        ...
        Args:
            ...
            source (string): 仅新闻类型有效。指定新闻平台，如微博、知乎、B站、抖音、快手、头条、百度、腾讯、小红书、夸克、36氪、51CTO、A站、爱范儿、网易、新浪、澎湃、第一财经、财联社。不指定则留空。
    ...
    async def qzone_tool(
        ...
        """
        查看、发布、点赞、评论 QQ 空间说说。
        当用户说“看我的空间”“看看QQ空间”“看说说”“发说说”“发到空间”“给这条说说点赞”“评论这条说说”等空间相关请求时调用。
        本工具不依赖额外安装 QQ 空间插件，会使用每日分享内置 QQ 空间服务。
        操作点赞、评论和发布前，应先通过 list 查看说说，拿到工具返回的说说 ID，再把 ID 填入 post_id。

        Args:
            action (string): 操作类型。查看说说填 list；查看详情填 detail；发布说说填 publish；点赞填 like；评论填 comment。
            post_id (string): 说说 ID，来自 list/detail 返回中的 ID，例如 123456:abcdef。点赞、评论、详情必填。
            target_id (string): 可选，要查看的 QQ 号。留空表示查看自己的 QQ 空间说说；填 QQ 号表示查看该 QQ 空间说说。
            content (string): 发布说说或评论的正文。publish/comment 必填。
            images (list): 发布说说附带的图片路径或图片 URL 列表，可留空。
            pos (number): 查看说说起始位置，默认 0。
            num (number): 查看说说数量，默认 5，最多 10。
        """
        return await self._qzone_tool_impl(
            event,
            action=action,
            post_id=post_id,
            target_id=target_id,
