import { LIGHTBOX_ANIMATION_MS } from "./limits.js";
import { createMediaGalleryController } from "./gallery.js";
import { createMediaImageController } from "./image.js";
import { createOverlayController } from "./overlay.js";
import { createLightboxVideoController } from "./player.js";
...
export function createMediaUi({
  ...
  function openMediaLightbox(src, _caption, kind = "image", imageSize = null) {
    ...
    if (wasHidden) mediaReturnFocus = document.activeElement;
  ...
  function closeMediaLightbox() {
    ...
    state.mediaLightboxCloseTimer = window.setTimeout(() => {
      ...
      if (mediaReturnFocus?.isConnected) mediaReturnFocus.focus();
  ...
  function bindMediaEvents() {
    ...
    el.textLightboxClose.addEventListener("click", (event) => {
      event.stopPropagation();
      overlay.closeTextLightbox();
    });
    document.addEventListener("keydown", (event) => {
      ...
      if (!el.textLightbox.hidden) overlay.trapDialogFocus(event, el.textLightbox);
      else if (!el.mediaLightbox.hidden) overlay.trapDialogFocus(event, el.mediaLightbox);
    });
  }
