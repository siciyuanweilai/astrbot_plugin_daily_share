...
import {
  currentMediaInitialLimit,
  MEDIA_LOAD_STEP,
  MEDIA_MAX_LIMIT,
} from "./limits.js";
import { createMediaItem, mediaKind } from "./items.js";
import {
  createMediaManageController,
  mediaHistoryId,
} from "./manage.js";
import { createMediaScrollController } from "./scroll.js";
...
export function createMediaGalleryController({
  ...
  function preferredInitialLimit() {
    return currentMediaInitialLimit();
  }

  function applyMediaPage(data = {}, { preferredLimit = null } = {}) {
    const items = Array.isArray(data.items)
      ? data.items
      : Array.isArray(data.media)
        ? data.media
        : [];
    const requestedLimit = Number(preferredLimit ?? data.limit ?? data.media_limit);
    const limit = Number.isFinite(requestedLimit) && requestedLimit > 0
      ? Math.min(requestedLimit, MEDIA_MAX_LIMIT)
      : preferredInitialLimit();
    const hasMore = data.has_more ?? data.media_has_more;
    state.media = items.slice(0, limit);
    state.mediaLimit = limit;
  ...
  function syncDefaultMediaFromStatus(status = {}) {
    ...
    const initialLimit = preferredInitialLimit();
    if (!state.mediaLoaded || state.mediaLimit <= initialLimit) {
      applyMediaPage(status, { preferredLimit: initialLimit });
      return;
    }
    ...
    const limit = Math.max(initialLimit, state.mediaLimit);
  ...
  function resetMediaPage() {
    ...
    state.mediaLimit = preferredInitialLimit();
  ...
  function renderMedia({ force = false } = {}) {
    ...
    const media = state.mediaLoaded
      ? state.media
      : (state.status?.media || []).slice(0, preferredInitialLimit());
  ...
  async function reloadMediaPage({ quiet = false } = {}) {
    ...
    renderMedia();
    try {
      const data = await apiGet("page/media", mediaPageParams(state.mediaLimit || preferredInitialLimit()));
