...
export function createQzoneRelation({
  ...
  function renderRelation() {
    ...
    for (const button of el.qzoneRelationTabs || []) {
      ...
      if (active && el.qzoneRelationGrid) {
        el.qzoneRelationGrid.setAttribute("aria-labelledby", button.id);
      }
    }
  ...
  function bindRelationEvents() {
    const tabs = el.qzoneRelationTabs || [];
    for (const [index, button] of tabs.entries()) {
      ...
      button.addEventListener("keydown", (event) => {
        ...
        switchRelation(nextTab.dataset.qzoneRelation);
        nextTab.focus();
      });
    }
