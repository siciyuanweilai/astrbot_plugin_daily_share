...
function createMediaItemControls(item, handlers) {
  const id = mediaHistoryId(item);
  if (!id || !handlers.selectMode) return null;
  ...
  button.type = "button";
  button.disabled = Boolean(handlers.deleting);
  ...
  if (handlers.selectMode) {
    button.className = "media-select-toggle";
    button.textContent = handlers.selected ? "✓" : "";
    button.setAttribute("aria-label", handlers.selected ? "取消选择记录" : "选择记录");
    button.setAttribute("aria-pressed", handlers.selected ? "true" : "false");
  } else {
    button.className = "media-select-toggle";
  ...
  if (!handlers.selectMode) {
    button.classList.toggle("is-confirming", Boolean(handlers.confirming));
    button.textContent = handlers.confirming ? "\u786e\u8ba4" : "\u5220\u9664";
    button.setAttribute("aria-label", handlers.confirming ? "\u786e\u8ba4\u5220\u9664\u8bb0\u5f55" : "\u5220\u9664\u8bb0\u5f55");
  }
...
export function createMediaItem(item, handlers = {}) {
  ...
  const historyId = mediaHistoryId(item);
  if (historyId) node.dataset.historyId = String(historyId);
  node.classList.toggle("is-selecting", Boolean(handlers.selectMode));
  node.classList.toggle("is-selected", Boolean(handlers.selected));
  ...
  const controls = createMediaItemControls(item, handlers);
